...
├── SKILL.md                          # Main skill definition (v2.1 navigation hub)
├── README.md                         # This file
├── phases/                           # Modular phase files (load on-demand)
│   ├── phase-summary.md             # Quick reference overview
│   ├── phase-0-ingestion.md         # Trackator JSON ingestion
│   ├── phase-1-intent-filtering.md  # FP filtering
│   ├── phase-2-pattern-matching.md  # Historical exploit matching
│   ├── phase-3-creative-attack.md   # Novel attack synthesis
│   ├── phase-4-fork-testing.md      # Mainnet fork validation
│   └── phase-5-reporting.md         # Final report generation
├── agents/
│   ├── hacker-agent.md               # Creative Hacker agent spec (v2.1 UPDATED)
│   └── verifier-agent.md             # Verifier agent spec (v2.1 UPDATED)
├── plugins/
│   ├── intended-behavior.md          # Phase 1: Intent filtering plugin
│   ├── pattern-matcher.md            # Phase 2: Historical exploit matching
│   ├── attack-chain-composer.md     # Phase 2: Attack chain assembly
│   ├── reachability.md               # Block gate plugin (Phases 2,3,4)
│   ├── reverse-engineering.md        # Phase 3: Value flow tracing
│   ├── assumption-breaker.md         # Phase 3: Trust assumption testing
│   ├── root-cause-hypothesizer.md    # Phase 3: Root cause analysis
│   ├── attacker-mindset-simulator.md # Phase 3: Attacker psychology
│   ├── fork-test.md                  # Phase 4: Mainnet fork validation
│   └── anti-pattern-library.md       # Phase 4: FP elimination
├── templates/
│   └── report-template.md            # Report output format (v2.1)
└── references/
    ├── trackator-fields.md           # Data structure definitions
    └── evidence-calibration.md       # Evidence classification logic
```
...
| Foundry | Latest | Fork testing (Phase 4) |
...
| 2 | Pattern Matching | Hacker → Verifier | Pattern matches + reachability | pattern-matcher.md, attack-chain-composer.md, reachability.md |
| 3 | Creative Attack | Hacker → Verifier | New attack ideas + traces | reverse-engineering.md, assumption-breaker.md, root-cause-hypothesizer.md, attacker-mindset-simulator.md, reachability.md |
| 4 | Fork Testing | Hacker → Verifier | Mainnet-confirmed exploits | fork-test.md, anti-pattern-library.md, reachability.md |
| 5 | Reporting | System | Final assessment report | report-template.md |

### Enhanced Data Layers (from Trackator)

| Trackator Phase | RedTeam Integration | Key Capabilities |
|---------------|-------------------|----------------|
| **Storage Dependency Analyzer** | Phase 3 (Reverse Eng.), Phase 4 (Fork) | Variable writers map, value-bearing variables, shared-state matrix, race condition detection |
| **State Coupling Detector** | Phase 3 (Coupling Analysis) | Function dependency N×N matrix, hidden couplings, invariant→function mapping, top attack intersections |
| **Sync Analyzer** | Phase 3 (Assumption Breaker), Phase 4 | Assumption dependency graph, stale data detection, critical desync risks (TOCTOU, oracle, race windows) |
| **Evidence Validator** | Phase 2 (Classification), Phase 4 (Proof) | 6-class classification system, 9-criteria reachability proof, confidence scoring |
...
confidence = (
    (patternMatch * 0.20) +
    (traceComplete * 0.20) +
    (forkSuccess * 0.50) +      // Includes former fuzz weight (v2.1 merge)
...
### Workflow 1: Standard Assessment (v2.1)

```
Phase 0 → Phase 1 → Phase 2 → Phase 3 → Phase 4 → Phase 5
```

### Workflow 2: Fast Track (No Fork)
...
### Workflow 3: Deep Dive (Extended Fork Iteration)

```
Phase 0 → ... → Phase 4 [with MAX_ITERATIONS=20] → Phase 5
...
**Symptom**: `❌ Fork RPC unavailable - skipping Phase 4`
...
└── fork-test-results.json         # Phase 4 output
```

## Plugin Quick Reference (v2.1)
...
| `attack-chain-composer.md` | 2 | Chain assembly | Composes multi-step attacks |
| `reachability.md` | 2, 3, 4 | Block gate validation | Saves findings instead of killing |
...
| `root-cause-hypothesizer.md` | 3 | Root cause analysis | Why > What analysis |
| `attacker-mindset-simulator.md` | 3 | Attacker psychology | Real attacker decision-making |
| `fork-test.md` | 4 | Mainnet validation | Iterative exploitation with visualization |
| `anti-pattern-library.md` | 4 | FP elimination | Historical audit failure patterns |

## Version History

| Version | Date | Changes |
|---------|------|---------|
| **2.1.0** | 2026-08-06 | **REFACTOR**: Modular file structure; Removed Fuzzing (separate skill); 5-phase pipeline; SKILL.md as navigation hub; Scoring weights updated (forkSuccess=50%) |
| 2.0.0 | 2026-07-30 | MAJOR: Trackator Enhanced Integration; Disproof Engine; 6-class classification |
...
- Review phase files for per-phase instructions
