...
exports.parseFoundryTrace = parseFoundryTrace;
exports.normalizeTraceFormat = normalizeTraceFormat;
exports.parseTransactionInfo = parseTransactionInfo;
exports.parseTraceSteps = parseTraceSteps;
exports.parseLogs = parseLogs;
exports.decodeFunctionSignature = decodeFunctionSignature;
exports.attemptLogDecoding = attemptLogDecoding;
const fs = __importStar(require("fs"));
/**
 * Parse Foundry trace JSON file
 */
function parseFoundryTrace(traceFilePath, options = {}) {
    const { verbose = false, maxDepth = 100, includeOpcodes = false } = options;
    if (verbose)
        console.log(`Parsing trace file: ${traceFilePath}`);
    // Read and parse JSON
    ...
    if (verbose)
        console.log('Raw trace loaded, parsing structure...');
 ...
 */
function normalizeTraceFormat(raw) {
...
function extractTransactionFromArray(traceArray) {
...
function convertDebugTrace(debugTrace) {
 ...
 */
function parseTransactionInfo(normalized) {
 ...
 */
function parseTraceSteps(normalized, options) {
...
function parseSingleStep(entry, index, depth, options) {
 ...
 */
function parseOpcodes(structLogs, baseDepth) {
    return structLogs.map((log, idx) => ({
        pc: log.pc || 0,
 ...
 */
function calculateGasFromOpcodes(entry) {
    if (!entry.structLogs)
        return 0;
    return entry.structLogs.reduce((total, log) => {
 ...
 */
function parseReceipts(normalized) {
    const receipts = [];
...
function parseSingleReceipt(receipt) {
    return {
...
function extractLogsFromTrace(traces, logs) {
 ...
 */
function attemptLogDecoding(log) {
 ...
 */
function computeGasInfo(_trace, receipt) {
...
function decodeFunctionSignature(calldata) {
...
//# sourceMappingURL=foundry-trace-parser.js.map