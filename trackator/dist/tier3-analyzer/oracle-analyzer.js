...
exports.analyzeOracles = analyzeOracles;
exports.createDefaultOracleConfigs = createDefaultOracleConfigs;
/**
 * Main entry point for oracle analysis
 */
function analyzeOracles(trace, options = {}) {
    const { knownOracles = new Map(), deviationThreshold = 5, twapWindow = 1800, // 30 minutes in seconds
 ...
 */
function collectPriceObservations(trace, knownOracles) {
    const observations = [];
...
function collectPricesFromStep(step, observations, processedCalls, knownOracles) {
    if (processedCalls.has(step.stepIndex))
        return;
 ...
 */
function isLikelyOracleCall(step) {
 ...
 */
function extractPriceFromCall(step, config) {
    try {
        let price = 0;
        let asset = config?.asset || 'unknown';
        let source = config?.type || 'custom';
 ...
 */
function decodeFunctionSelector(calldata) {
 ...
 */
function computePriceDeviations(observations, thresholdPercent) {
    const deviations = [];
 ...
 */
function computeTWAP(observations, _windowSeconds) {
 ...
 */
function detectManipulationPatterns(trace, _observations, deviations) {
    const indicators = [];
 ...
 */
function detectFlashLoanPattern(trace) {
 ...
 */
function detectLiquidationManipulation(trace, deviations) {
 ...
 */
function detectOracleFlip(deviations) {
 ...
 */
function detectStalePrice(_trace) {
 ...
 */
function createDefaultOracleConfigs() {
...
//# sourceMappingURL=oracle-analyzer.js.map