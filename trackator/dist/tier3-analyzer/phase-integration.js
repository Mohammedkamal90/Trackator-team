...
exports.runUnifiedAnalysis = runUnifiedAnalysis;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const storage_dependency_analyzer_1 = require("../tier2-enricher/storage-dependency-analyzer");
const state_coupling_detector_1 = require("../tier2-enricher/state-coupling-detector");
const sync_analyzer_1 = require("../tier2-enricher/sync-analyzer");
const evidence_validator_1 = require("./evidence-validator");
const state_coupling_detector_2 = require("../tier2-enricher/state-coupling-detector");
// ============================================================
 ...
 */
async function runUnifiedAnalysis(options) {
    const startTime = Date.now();
    const { contracts, functionRegistry, callEdges = [], invariants = [], runPhase1 = true, runPhase2 = true, runPhase3 = true, runPhase4 = true, outputDir, verbose = false } = options;
    console.log('');
    console.log('╔══════════════════════════════════════════════════════════════╗');
    ...
    if (runPhase1) {
        ...
        try {
            ...
            result.phase1 = (0, storage_dependency_analyzer_1.analyzeStorageDependencies)(phase1Opts);
    ...
    if (runPhase2) {
        ...
        try {
            ...
            result.phase2 = (0, state_coupling_detector_1.analyzeStateCouplings)(phase2Opts);
    ...
    if (runPhase3) {
        ...
        try {
            ...
            result.phase3 = (0, sync_analyzer_1.analyzeSynchronization)(phase3Opts);
    ...
    if (runPhase4) {
        ...
        try {
            ...
            const phase3Assumptions = (result.phase3?.assumptionDependencyGraph.nodes || []);
            const phase4Opts = {
                contracts,
                functionRegistry,
                callEdges,
            ...
            result.phase4 = (0, evidence_validator_1.validateEvidence)(phase4Opts);
    ...
    if (runPhase4 && result.phase4) {
        ...
        console.log('└─────────────────────────────────────────────────────────────┘');
        try {
            result.calibratedConfidence = (0, evidence_validator_1.calibrateConfidence)(result.phase4.confidenceAssessments, result.phase4.classificationRegistry, result.phase4.reachabilityAnalysis, result.phase4.disproofAnalysis, {
    ...
    if (runPhase2 && result.phase2) {
        ...
        console.log('└─────────────────────────────────────────────────────────────┘');
        try {
            result.weaponizedAttacks = (0, state_coupling_detector_2.generateWeaponizedAttacks)(result.phase2, {
                storageWriteGraph: result.phase1?.storageWriteGraph,
                verbose
            });
...
// ============================================================
function generateUnifiedRecommendations(result) {
    const recs = [];
...
// ============================================================
function printUnifiedSummary(result) {
    console.log('');
    console.log('╔══════════════════════════════════════════════════════════════╗');
 ...
 */
function saveAdvancedOutputs(result, outputDir) {
    if (!outputDir)
        return;
    ...
    if (result.calibratedConfidence) {
        ...
        const calibratedOutput = (0, evidence_validator_1.exportCalibratedResults)(result.calibratedConfidence);
    ...
    if (result.weaponizedAttacks) {
        ...
        const attacksOutput = {
            ...
            criticalAttackScenarios: wa.attackScenarios.filter(a => a.severity === 'critical' || a.severity === 'high'),
