...
exports.AIEnrichmentEngine = void 0;
...
class AIEnrichmentEngine {
    ...
    generateEnrichedAnalysis() {
        const analysis = {
            ...
            complexity: typeof this.xRayReport?.protocolClassification === 'string'
                ...
                : this.xRayReport?.protocolClassification?.complexity || 'medium',
        ...
        for (const surface of attackSurfaces) {
            ...
            if (riskLevel === 'critical' || riskLevel === 'high') {
                rules.push({
                    ...
                    remediation: Array.isArray(surface.mitigations) ? surface.mitigations.join('; ') : 'Implement proper access controls.',
                    enabled: true
                });
            }
        }
    ...
    generateTrustAssumptionRules() {
        const rules = [];
        if (!this.xRayReport?.trustAssumptions)
            return rules;
        let index = 200;
    ...
    generateExpectedBehaviorRules() {
        const rules = [];
        if (!this.breakdownReport?.expectedBehaviors)
            return rules;
        let index = 300;
    ...
    generateRolePermissionRules() {
        ...
        for (const rolePerm of rolePermissions) {
            ...
            if (typeof damagePotential === 'string' && (damagePotential.includes('drain') ||
    ...
    identifyHighRiskAreas() {
        ...
        const ac = this.structure.accessControl;
        ...
        const ecm = this.structure.externalCallMap;
    ...
    structureMoneyFlows() {
        ...
        const flows = [];
        const contracts = this.structure.contracts || [this.structure];
        for (const contract of contracts) {
            ...
            for (const func of functions) {
                ...
                const hasTransfer = externalCalls.some((c) => c.isSendOrTransfer || c.isSend || c.functionName?.toLowerCase() === 'transfer' || c.name?.toLowerCase() === 'transfer');
                if (hasTransfer) {
                    flows.push({
    ...
    // ==================== HELPERS ====================
    createConditionFromString(expr) {
    ...
    parseBreakdownMarkdown(content) {
        // Simplified parser
        return {
            components: [],
            coreComponents: [],
    ...
    extractInvariantsFromMD(content) {
        const invariants = [];
        const regex = /(?:^|\n)[-*]\s*\*\*(INV-\d+)?\**\s*(.+?)(?:\n|$)/gi;
        let match;
        let index = 0;
    ...
    extractExpectedBehaviorsFromMD(content) {
        ...
        return [
            {
                ...
                stepByStep: [],
                anomaliesToDetect: [
                    { id: 'ANOM-001', name: 'Unexpected state change', description: '', detectionLogic: { type: 'custom', field: 'state_change', operator: 'changed', value: true }, severity: 'medium', category: 'anomaly' }
                ]
            }
        ];
    }
     ...
     */
    exportAlertRules(rules, outputPath) {
        const yamlContent = yaml.dump({ alert_rules: rules }, { indent: 2, lineWidth: 120 });
        fs.writeFileSync(outputPath, yamlContent, 'utf-8');
        console.log(`✅ Alert rules exported: ${outputPath}`);
    }
}
exports.AIEnrichmentEngine = AIEnrichmentEngine;
exports.default = AIEnrichmentEngine;
//# sourceMappingURL=AIEnrichmentEngine.js.map