...
exports.generateMermaidCallGraph = generateMermaidCallGraph;
exports.generateSequenceDiagram = generateSequenceDiagram;
exports.generateClassDiagram = generateClassDiagram;
exports.exportCallGraph = exportCallGraph;
/**
 * Build complete call graph from parsed contracts
 */
function buildCallGraph(contracts) {
    const nodes = new Map();
    const edges = [];
    ...
    for (const contract of contracts) {
        for (const func of contract.functions) {
            ...
            for (const calledFuncName of func.calls) {
                const edge = resolveCallEdge(contract.name, func.name, calledFuncName, contracts, func);
                if (edge) {
                    edges.push(edge);
 ...
 */
function resolveCallEdge(fromContract, fromFunction, calledName, contracts, callerFunc) {
    ...
    if (dotIndex > 0) {
        ...
        const isExternalContract = contracts.some(c => c.name === potentialContract ||
 ...
 */
function analyzeValueFlow(callerFunc, calledName) {
 ...
 * Analyze external interface calls (e.g., IERC20(token).transferFrom)
 */
function analyzeExternalInterfaceCalls(contract, func, edges, nodes) {
...
function computeDepths(nodes) {
    ...
    const computeDepth = (nodeKey, visiting) => {
 ...
 */
function generateMermaidCallGraph(graph) {
 ...
 */
function generateSequenceDiagram(graph, startNode, maxDepth = 5) {
    const lines = ['sequenceDiagram'];
    const participants = new Set();
    const visited = new Set();
    const addCalls = (nodeKey, depth) => {
        if (depth > maxDepth || visited.has(nodeKey))
            return;
        visited.add(nodeKey);
 ...
 */
function generateClassDiagram(contracts) {
    ...
    for (const contract of contracts) {
        ...
        for (const event of contract.events) {
            const params = event.parameters.map(p => p.indexed ? `${p.type} indexed ${p.name}` : p.type).join(', ');
...
// UTILITY FUNCTIONS
// ============================================================
function groupNodesByContract(nodes) {
 ...
 */
function exportCallGraph(graph) {
    const nodesArray = [];
    for (const [key, node] of graph.nodes) {
        nodesArray.push({
            key,
