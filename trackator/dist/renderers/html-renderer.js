 ...
 */
function generateHtmlReport(outputDir, trace, stateDiffs, alerts, oracleAnalysis, roleJourneys, summary, options = {}) {
    const { includeCharts = true, includeInteractivity = true, theme = 'dark' } = options;
...
function generateHtmlSkeleton(theme) {
...
function generateHeaderSection(trace, summary) {
...
function generateDashboardTab(trace, summary, alerts, oracleAnalysis) {
    return `
...
function generateExecutionFlowTab(trace) {
    return `
...
function generateExecutionTreeHTML(steps, depth) {
    let html = '';
...
function generateStateChangesTab(stateDiffs) {
    let slotRows = '';
...
function generateAlertsTab(alerts) {
    let alertItems = '';
    if (alerts.length === 0) {
...
function generateOracleTab(analysis) {
    let priceRows = '';
...
function generateRoleJourneysTab(journeys) {
    return `
...
function generateSidebar(hasData, hasOracle, hasRoles) {
    return `
<aside class="sidebar">
...
function generateJavaScript(_trace, _stateDiffs, _alerts, _oracleAnalysis) {
    return `
<script>
// Tab switching
...
function generateChartIncludes() {
    return `
