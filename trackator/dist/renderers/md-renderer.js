 ...
 */
function generateMarkdownReport(outputDir, trace, stateDiffs, alerts, oracleAnalysis, roleJourneys, summary, options = {}) {
    const { includeMermaid = true, includeDetails = true, maxAlerts = 50 } = options;
    let md = '';
    // Header
    ...
    md += '- [Alerts](#alerts)\n';
    if (oracleAnalysis)
        md += '- [Oracle Analysis](#oracle-analysis)\n';
    if (roleJourneys?.length)
        md += '- [Role Journeys](#role-journeys)\n';
    md += '- [Verdict](#verdict)\n\n';
    // Executive Summary
...
function generateExecutiveSummary(summary, alerts) {
    let md = '';
    if (!summary) {
...
function generateTransactionTable(trace) {
...
function generateExecutionMermaid(trace) {
    let md = '### Call Graph\n\n';
    md += '```mermaid\n';
    md += 'graph TD\n';
...
function generateMermaidNodes(steps, depth) {
    let lines = '';
...
function generateExecutionList(trace) {
...
function generateStateChangesSection(diffs, includeDetails) {
    let md = '';
    // Summary table
...
function generateAlertsSection(alerts, maxShow) {
    let md = '';
    if (alerts.length === 0) {
...
function generateOracleSection(analysis) {
    let md = '';
...
function generateRoleJourneysSection(journeys) {
    let md = '';
...
function generateVerdictSection(summary, alerts) {
    let md = '';
