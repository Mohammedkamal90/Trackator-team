 ...
 */
function ingestXRay(options) {
    const { xrayFile, protocolType: overrideProtocolType, contracts = [], verbose = false } = options;
    // Try to load from file if provided
    let rawData = null;
    if (xrayFile && fs.existsSync(xrayFile)) {
        if (verbose)
            console.log(`Loading X-Ray data from ${xrayFile}`);
        rawData = loadXRayFile(xrayFile);
    }
    // If no file, generate from contracts using built-in templates
    if (!rawData && contracts.length > 0) {
        if (verbose)
            console.log('Generating X-Ray analysis from contract templates');
 ...
 */
function loadXRayFile(filePath) {
 ...
 */
function generateXRayFromContracts(contracts, overrideProtocolType) {
 ...
 */
function detectProtocolType(contracts) {
 ...
 */
function extractAssets(contracts) {
    const assets = [];
 ...
 */
function extractEntryPoints(contracts) {
    const entryPoints = [];
    for (const contract of contracts) {
 ...
 */
function parseXRayData(rawData, overrideProtocolType, contracts = []) {
    const protocolType = overrideProtocolType || rawData.protocolType || detectProtocolType(contracts);
    // Parse threat model
    const threatModel = parseThreatModel(rawData.threatModel || {}, contracts);
    // Parse invariants
    const invariants = parseInvariants(rawData.invariants || [], protocolType, contracts);
    // Parse trust assumptions
    const trustAssumptions = parseTrustAssumptions(rawData.trustAssumptions || [], protocolType);
    // Parse adversary profiles
    const adversaryProfiles = parseAdversaryProfiles(rawData.adversaryProfiles || rawData.adversaries || [], protocolType);
    // Parse attack vectors
    const attackVectors = parseAttackVectors(rawData.attackVectors || rawData.threats || [], protocolType);
    return {
        protocolType,
        threatModel,
        invariants,
...
function parseThreatModel(raw, contracts) {
    return {
...
function parsePrivilegeBoundaries(raw) {
    return raw.map((b) => ({
        from: b.from || '',
        to: b.to || '',
...
function parseInvariants(raw, protocolType, contracts) {
    // If we have raw invariants, parse them
    if (raw.length > 0) {
        return raw.map((inv, index) => ({
...
function parseTrustAssumptions(raw, protocolType) {
    const parsed = raw.map((ta, index) => ({
    ...
    if (parsed.length === 0) {
        parsed.push(...getTrustAssumptionsForProtocol(protocolType));
    }
    return parsed;
}
function parseAdversaryProfiles(raw, protocolType) {
    if (raw.length > 0) {
        return raw.map((ap, index) => ({
...
function parseAttackVectors(raw, protocolType) {
    if (raw.length > 0) {
        return raw.map((av, index) => ({
...
// ============================================================
function generateTemplateInvariants(protocolType, contracts) {
    const invariants = [];
    let counter = 1;
...
function getLendingInvariants(startId) {
    const inv = [];
    let id = startId;
    inv.push({
        id: `INV_${id++}`,
...
function getDexInvariants(startId) {
    const inv = [];
    let id = startId;
    inv.push({
        id: `INV_${id++}`,
...
function getVaultInvariants(startId) {
    const inv = [];
    let id = startId;
    inv.push({
        id: `INV_${id++}`,
...
function getBridgeInvariants(startId) {
    const inv = [];
    let id = startId;
    inv.push({
        id: `INV_${id++}`,
...
function getPerpDexInvariants(startId) {
    const inv = [];
    let id = startId;
    inv.push({
        id: `INV_${id++}`,
...
function getLSDInvariants(startId) {
    const inv = [];
    let id = startId;
    inv.push({
        id: `INV_${id++}`,
...
function getTrustAssumptionsForProtocol(protocolType) {
    const assumptions = [
        {
            id: 'TA_1',
            category: 'oracle',
...
function getAdversariesForProtocol(protocolType) {
    const baseAdversaries = [
        {
            id: 'ADV_EXT',
            type: 'external',
...
function getThreatsForProtocol(protocolType) {
    const threats = [];
    let counter = 1;
    // Universal threats
    threats.push({
...
function createEmptyXRayOutput() {
    return {
...
//# sourceMappingURL=xray-ingestor.js.map