...
exports.runEnrichment = runEnrichment;
exports.getAlertRulesBySeverity = getAlertRulesBySeverity;
exports.getAlertRulesByCategory = getAlertRulesByCategory;
exports.exportEnrichmentResult = exportEnrichmentResult;
const xray_ingestor_1 = require("./xray-ingestor");
const breakdown_ingestor_1 = require("./breakdown-ingestor");
/**
 * Main entry point for Tier 2 enrichment
 */
function runEnrichment(options) {
    const { xrayFile, breakdownFile, protocolType, contracts = [], registry, generateAlerts = true, verbose = false } = options;
    if (verbose)
        console.log('Starting Tier 2 enrichment...');
    // Step 1: Ingest X-Ray data
    const xray = (0, xray_ingestor_1.ingestXRay)({
        xrayFile,
        protocolType,
        contracts,
        verbose
    ...
    if (verbose)
        console.log(`X-Ray loaded: ${xray.protocolType} protocol, ${xray.invariants.length} invariants`);
    // Step 2: Ingest Breakdown data
    const breakdown = (0, breakdown_ingestor_1.ingestBreakdown)({
        breakdownFile,
        contracts,
        verbose
    });
    if (verbose)
        console.log(`Breakdown loaded: ${breakdown.components.length} components, ${breakdown.moneyFlows.length} flows`);
    ...
    if (generateAlerts) {
        ...
        if (verbose)
            console.log(`Generated ${alertRules.length} alert rules`);
    }
    return {
        xray,
        breakdown,
 ...
 */
function mergeInvariants(xray, breakdown) {
 ...
 */
function generateAlertRules(xray, breakdown, invariants, registry) {
    const rules = [];
    let ruleCounter = 1;
    // ========================================
    ...
    for (const threat of xray.attackVectors.filter(t => t.category === 'flash-loan')) {
        rules.push({
            ...
            condition: {
                ...
                value: ['large_deposit', 'manipulation_action', 'large_withdraw']
            },
 ...
 */
function enhanceRulesWithRegistry(rules, registry) {
    ...
    for (const [, funcs] of registry) {
        for (const func of funcs) {
            ...
            if (func.risk?.overall === 'critical') {
                ...
                const existingAlert = rules.find(r => r.name.includes(funcName) &&
                ...
                if (!existingAlert && body.ceiPattern === 'violated') {
                    ...
                    rules.push({
                        ...
                        description: `High-risk function ${funcName}() violates CEI pattern. Risk factors: ${riskFactors.map((f) => f.type).join(', ')}.`,
 ...
 */
function getAlertRulesBySeverity(rules, minSeverity = 'info') {
    const severityOrder = {
        'critical': 5,
        'high': 4,
        'medium': 3,
    ...
    return rules.filter(r => r.enabled && (severityOrder[r.severity] || 0) >= minOrder);
}
/**
 * Get alert rules by category
 */
function getAlertRulesByCategory(rules, category) {
    return rules.filter(r => r.enabled && r.category === category);
}
/**
 * Export enrichment result to JSON-serializable format
 */
function exportEnrichmentResult(result) {
    return {
        xray: result.xray,
...
//# sourceMappingURL=invariant-generator.js.map