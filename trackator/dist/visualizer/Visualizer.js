...
class Visualizer {
     ...
     */
    generateTerminalOutput(verbose = false) {
        const lines = [];
        // Header
...
╰──────────────────────────────────────────────────╯`;
    ...
    generateSummaryStats() {
        ...
        return `┌─ SUMMARY ─────────────────────────────────────┐
│                                                  │
│  Value-bearing variables: ${chalk_1.default.green(String(valueBearing).padStart(4))}                 │
...
└──────────────────────────────────────────────────┘`;
    ...
    generateContractDetails() {
        ...
        for (const contract of this.structure.contracts) {
            if (contract.kind === 'contract' || contract.kind === 'abstract') {
                ...
                lines.push(`│                                                  │`);
                lines.push(`│  ${kindIcon} ${chalk_1.default.bold(contract.name)} (${contract.kind})`);
    ...
    generateStateInventoryASCII() {
        ...
        for (const [contract, vars] of Array.from(this.structure.stateInventory.byContract.entries())) {
            ...
            if (stateVars.length === 0)
                continue;
            lines.push(`│                                                  │`);
            lines.push(`│  ${chalk_1.default.bold(contract)}`);
     ...
     */
    generateAccessControlASCII() {
        const lines = ['┌─ ACCESS CONTROL MATRIX ───────────────────────┐'];
        if (this.structure.accessControl.roles.length === 0) {
            ...
            for (const role of this.structure.accessControl.roles) {
                ...
                lines.push(`│                                                  │`);
                lines.push(`│  ${trustIcon} ${chalk_1.default.bold(role.name)} (${role.trustLevel.toUpperCase()})`);
     ...
     */
    generateDangerousPatternsASCII() {
        const lines = ['┌─ ⚠️  DANGEROUS PATTERDS ───────────────────────┐'];
    ...
    generateArchitectureDiagram() {
        ...
        const coreContracts = this.structure.contracts.filter(c => c.kind === 'contract' || c.kind === 'abstract');
     ...
     */
    generateUserFlowDiagram(flowName, steps) {
     ...
     */
    generateMoneyFlowDiagram(flows) {
        const lines = [
            `flowchart LR`,
