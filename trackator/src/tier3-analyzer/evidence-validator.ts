...
import { 
  StorageWriteGraph,
  HighRiskFinding as StorageHighRiskFinding
} from '../tier2-enricher/storage-dependency-analyzer';
import { 
  CriticalFinding as CouplingCriticalFinding,
  HiddenAssumption
} from '../tier2-enricher/state-coupling-detector';
import { 
  DesyncRisk as SyncDesyncRisk
} from '../tier2-enricher/sync-analyzer';

// ============================================================
// CORE TYPE DEFINITIONS FOR EVIDENCE VALIDATION
// ============================================================

export interface EvidenceValidationResult {
  timestamp: string;
  classificationRegistry: ClassificationRegistry;
  reachabilityAnalysis: ReachabilityAnalysisReport;
  disproofAnalysis: DisproofReport;
  confidenceAssessments: ConfidenceAssessment[];
  // FIX (integration bug): this was computed internally (Step 5 of validateEvidence,
  // fed into generateFinalVerdict) but never surfaced on the returned object or exported
  // to trackator-evidence.json — the touted "9-criteria reachability proof" was silently
  // discarded on every run. redteam-trackator's phase-4-fork-testing.md Step 5 reads
  // `context.evidence.proofRequirementsList` and always no-op'd as a result.
  proofRequirementsList: ProofRequirements[];
  finalVerdict: FinalVerdictTable;
...
export type FindingClassification =
  | 'proven-property'           // ✅ Proven by code evidence
  | 'potential-bug'             // 🟡 Could be a bug but unproven
  | 'reachable-bug'             // 🔴 Confirmed reachable by permissionless actor
  | 'false-positive'            // ⚪ Appears problematic but is actually safe/by-design
...
export interface ClassifiedFinding {
  ...
  confidence: number;              // 0-100 for this classification
  
  // Evidence
  supportingEvidence: EvidenceItem[];
  blockingEvidence: EvidenceItem[];   // Evidence that PREVENTS exploitation
  ...
  supersededBy?: string;          // If another finding supersedes this one
...
export interface EvidenceItem {
  itemId: string;
  itemType: 'code-pattern' | 'execution-path' | 'state-access' | 'external-call' | 'invariant-violation' | 'mathematical' | 'heuristic' | 'validation' | 'access-control' | 'protocol-pattern' | 'economic' | 'state-prerequisite';
  description: string;
  strength: 'strong' | 'moderate' | 'weak';
  location?: FindingLocation;
  codeSnippet?: string;
  isSupporting: boolean;           // True = supports bug theory, False = blocks it
}
...
// -----------------------------------------------------------
// PART 2: REACHABILITY ANALYSIS TYPES
// -----------------------------------------------------------

export interface ReachabilityAnalysisReport {
  paths: ExecutionPath[];
  unreachableFindings: UnreachableFinding[];
...
export interface ExecutionPath {
  ...
  // State prerequisites
  statePrerequisites: StatePrerequisite[];
  
  // Cross-contract prerequisites
  crossContractPrereqs: CrossContractPrerequisite[];
  ...
  blockingRequirement?: BlockingRequirement;  // What prevents reachability
...
export interface ExecutionStep {
  ...
  function: string;
  contract: string;
  action: 'entry' | 'call' | 'state-read' | 'state-write' | 'external-call' | 'check' | 'return' | 'revert';
  target?: string;               // What's being called/read/written
  successCondition?: string;      // What must succeed
  failureMode?: string;           // What happens on failure
  gasCost?: string;              // Estimated gas for this step
}

export interface StatePrerequisite {
  variable: string;
  contract: string;
  requiredValue: string;          // e.g., "> 0", "== address", "any"
  currentValue?: string;         // If known
  canBeSatisfied: boolean;
  howToSatisfy: string;
}

export interface CrossContractPrerequisite {
  targetContract: string;
  requiredState: string;
  dependencyType: 'must-exist' | 'must-have-balance' | 'must-not-be-paused' | 'specific-version';
  canBeSatisfied: boolean;
}

export interface ExternalAssumption {
  assumption: string;
  category: 'oracle' | 'token-contract' | 'price-feed' | 'governance' | 'other';
  mustHold: boolean;
  realisticToViolate: boolean;
  violationDifficulty: 'easy' | 'moderate' | 'difficult';
}
...
export interface UnreachableFinding {
  ...
  description: string;
  couldBecomeReachable: string;   // What would make it reachable
}

export type UnreachabilityReason =
  | 'no-permissionless-entry'
  | 'access-control-blocks'
  | 'state-prerequisite-impossible'
  | 'cross-contract-prerequisite-fails'
  | 'invariant-prevents'
  | 'external-dependency-unrealistic'
  | 'gas-limitation'
...
export interface PrerequisiteMap {
  byFinding: Map<string, ExecutionPath[]>;
  commonPrerequisites: CommonPrerequisite[];
  satisfiedCount: number;
  unsatisfiedCount: number;
}

export interface CommonPrerequisite {
  prerequisite: string;
  findingsRequiring: string[];    // Finding IDs that need this
  satisfactionLevel: 'usually-satisfied' | 'sometimes-satisfied' | 'rarely-satisfied' | 'unknown';
}

export interface BlockingRequirement {
  requirement: string;
  type: 'access-control' | 'state' | 'invariant' | 'external' | 'economic';
  whyBlocking: string;
...
export interface ReachabilitySummary {
  ...
  unreachableFindings: number;
  triviallyExploitable: number;
...
// -----------------------------------------------------------
// PART 3: DISPROOF ENGINE TYPES
// -----------------------------------------------------------

export interface DisproofReport {
  disproofAttempts: DisproofAttempt[];
  results: DisproofResult[];
  summary: DisproofSummary;
}
...
export interface DisproofAttempt {
  ...
  executed: boolean;
  successful: boolean;            // True = found disproving evidence
  evidenceFound: EvidenceItem[];
  conclusion: string;
  timestamp: string;
}

export type DisproofStrategy =
  | 'hidden-validation-check'      // Look for validation we missed
  | 'implicit-invariant'           // Check if invariant implicitly prevents it
  | 'later-correction'            // Does later code fix the issue?
  | 'rollback-behavior'           // Does the transaction revert?
  | 'freshness-check'             // Is data freshness verified?
  | 'accounting-reconciliation'   // Is accounting reconciled later?
  | 'access-control-verification' // Is there AC we didn't detect?
  | 'protocol-level-guarantee'     // Does protocol architecture prevent it?
  | 'boundary-condition'           // Are boundary conditions safe?
  | 'economic-analysis';          // Is it economically feasible to exploit?

export interface DisproofResult {
  resultId: string;
  findingId: string;
  originalClassification: FindingClassification;
  newClassification: FindingClassification;
  disproofStrategy: DisproofStrategy;
  disproofEvidence: EvidenceItem[];
  reasoning: string;
  confidence: number;              // Confidence in this reclassification
}

export interface DisproofSummary {
  totalAttempts: number;
  successfulDisproofs: number;    // Found to be false positive or by-design
  confirmedBugs: number;          // Survived disproof attempts
  reclassifiedAsFP: number;
  reclassifiedAsByDesign: number;
  stillUncertain: number;
  mostEffectiveStrategies: Record<string, number>;
}

// -----------------------------------------------------------
// PART 4: PROOF REQUIREMENTS TYPES
// -----------------------------------------------------------

export interface ProofRequirements {
  findingId: string;
  requirements: ProofRequirement[];
  overallStatus: 'proven-reachable' | 'not-proven' | 'disproven' | 'insufficient-evidence';
  metRequirements: number;
  totalRequirements: number;
  missingRequirements: ProofRequirement[];
}

export interface ProofRequirement {
  reqId: string;
  findingId?: string;              // Optional: associated finding ID for lookup
  requirement: string;
  category: ProofReqCategory;
  status: 'met' | 'not-met' | 'partial' | 'unknown' | 'n/a' | 'proven-reachable' | 'disproven' | 'insufficient-evidence' | 'not-proven';
  evidence?: EvidenceItem;
  explanation: string;
}

export type ProofReqCategory =
  | 'entry-point-exists'          // Req 1: Permissionless entry point exists
  | 'execution-path-exists'       // Req 2: Reachable execution path exists
  | 'state-prerequisites'        // Req 3: Required state can occur
  | 'storage-prerequisites'      // Req 4: Required storage values can occur
  | 'no-validation-blocks'        // Req 5: No validation blocks the path
  | 'no-invariant-prevents'        // Req 6: No invariant prevents exploitation
  | 'no-reconciliation'           // Req 7: No reconciliation restores before impact
  | 'observable-impact'           // Req 8: Observable security impact exists
  | 'poc-constructible'          // Req 9: Realistic PoC can be constructed

// -----------------------------------------------------------
// PART 5: CONFIDENCE ASSESSMENT TYPES
// -----------------------------------------------------------
...
export interface ConfidenceAssessment {
  ...
  // Recommendation - uses RecommendedAction type for consistency with verdict entries
  recommendation: RecommendedAction;
  nextSteps: string[];
}

export interface ScoreBreakdown {
  evidenceScore: ComponentScore;
  reachabilityScore: ComponentScore;
  impactScore: ComponentScore;
  consistencyScore: ComponentScore;  // Internal consistency of finding
}
...
export interface ScoreFactor {
  factor: string;
  weight: number;                 // Contribution to component
  achieved: number;                // 0-100 for this factor
  notes: string;
}
...
export interface UnknownFactor {
  factor: string;
  whyUnknown: string;
  impactIfWrong: string;          // How would affect classification
  suggestedInvestigation: string;
}

// -----------------------------------------------------------
// PART 6: FINAL VERDICT TABLE TYPES
// -----------------------------------------------------------

export interface FinalVerdictTable {
...
export interface VerdictEntry {
  entryId: string;
  findingId: string;
  observation: string;            // Original observation
  classification: FindingClassification;
...
export type FinalVerdict =
  | 'confirmed-vulnerability'    // Accept as real bug
  | 'potential-vulnerability'    // Needs more investigation
...
export interface ValidationSummary {
  totalFindingsClassified: number;
  classificationDistribution: Record<FindingClassification, number>;
  reachableBugs: number;
  unreachableFindings: number;
  falsePositiveRate: number;       // Estimate based on disproof analysis
  averageConfidence: number;
  mostCommonCategory: string;
  recommendations: ValidationRecommendation[];
  confirmedVulns: number;          // Confirmed vulnerability count
}
...
export interface EvidenceValidationOptions {
  ...
  phase1Findings?: StorageHighRiskFinding[];
  phase2Findings?: CouplingCriticalFinding[];
  phase3Risks?: SyncDesyncRisk[];
  phase3Assumptions?: HiddenAssumption[];
...
export function validateEvidence(
  ...
  if (verbose) console.log('[EvidenceValidator] Starting comprehensive evidence validation...');
  ...
  const reachabilityAnalysis = analyzeReachability(
    classificationRegistry.entries.filter(f => 
      f.classification === 'potential-bug' || f.classification === 'reachable-bug'
    ),
    contracts,
    functionRegistry,
    callEdges,
  ...
  const disproofAnalysis = runDisproofEngine(
    classificationRegistry.entries.filter(f => 
      f.classification !== 'insufficient-evidence'
    ),
    contracts,
    functionRegistry,
    callEdges,
  ...
  const confidenceAssessments = assessAllConfidence(
  ...
  const proofRequirementsList = generateProofRequirements(
    classificationRegistry.entries.filter(f =>
      f.classification === 'potential-bug' || f.classification === 'reachable-bug'
    ),
    reachabilityAnalysis,
  ...
  // Compile summary
  const summary = compileValidationSummary(
    classificationRegistry,
    reachabilityAnalysis,
    disproofAnalysis,
    confidenceAssessments,
...
function classifyAllFindings(
  ...
  if (verbose) console.log('[EvidenceValidator] Classifying all findings...');

  const entries: ClassifiedFinding[] = [];
  ...
  for (const entry of entries) {
    ...
    if (entry.confidence >= 80) highConfCount++;
    if (entry.confidence < 50) lowConfCount++;
  }
...
function initialClassifyPhase1Finding(
  finding: StorageHighRiskFinding,
  _contracts: SolidityContract[],
  _registry?: Map<string, RegisteredFunction[]>
...
function initialClassifyPhase2Finding(
  finding: CouplingCriticalFinding,
  _contracts: SolidityContract[],
  _registry?: Map<string, RegisteredFunction[]>
): InitialClassification {
  switch (finding.type) {
...
function initialClassifyPhase3Risk(
  risk: SyncDesyncRisk,
  _contracts: SolidityContract[],
  _registry?: Map<string, RegisteredFunction[]>
): InitialClassification {
...
function initialClassifyAssumption(
...
function mapToCategory(type: string): FindingCategory {
...
function analyzeReachability(
  ...
  invariants: Invariant[] = [],
  verbose: boolean = false
): ReachabilityAnalysisReport {
  if (verbose) console.log('[EvidenceValidator] Analyzing reachability...');

  const paths: ExecutionPath[] = [];
  const unreachableFindings: UnreachableFinding[] = [];
  const prereqMap = new Map<string, ExecutionPath[]>();
...
function buildExecutionPath(
  ...
  if (!func) {
    return {
      ...
      findingId: finding.findingId,
      entryPoint: { function: funcName || 'unknown', contract: contractName || 'unknown', visibility: 'unknown', accessControl: 'public' as AccessControlLevel, isPermissionless: false },
      callChain: [],
      statePrerequisites: [],
  ...
  const statePrerequisites: StatePrerequisite[] = [];
  ...
  const crossContractPrereqs: CrossContractPrerequisite[] = [];
  for (const extCall of externalCalls) {
  ...
  const externalAssumptions: ExternalAssumption[] = [];
  ...
  let blockingRequirement: BlockingRequirement | undefined;

  // Check 1: Must have permissionless entry
  if (!isPerm) {
    const hasRestriction = func.modifiers.some(m => 
      /onlyOwner|onlyAdmin|onlyRole|require/.test(m)
    );
  ...
  let exploitationComplexity: ExecutionPath['exploitationComplexity'];
  if (!isReachable) {
    exploitationComplexity = 'impossible';
...
function getEntrySuccessCondition(func: FunctionDef, _registered?: RegisteredFunction): string {
...
function getEntryFailureMode(func: FunctionDef, _registered?: RegisteredFunction): string {
...
function determineUnreachabilityReason(path: ExecutionPath): UnreachabilityReason {
  if (!path.isReachable) {
    if (path.reachabilityReason.includes('access control')) return 'access-control-blocks';
    if (path.reachabilityReason.includes('view') || path.reachabilityReason.includes('pure')) return 'no-permissionless-entry';
    if (path.reachabilityReason.includes('not found')) return 'function-not-found';
  }
  return 'unknown';
}

function suggestHowToMakeReachable(path: ExecutionPath): string {
  if (path.blockingRequirement?.type === 'access-control') {
...
function computeCommonPrerequisites(prereqMap: Map<string, ExecutionPath[]>): CommonPrerequisite[] {
  const prereqStrings = new Map<string, string[]>();
  ...
  const commonPrereqs: CommonPrerequisite[] = [];
...
function findMostCommonBlocker(unreachable: UnreachableFinding[]): string {
...
function runDisproofEngine(
  ...
  if (verbose) console.log('[EvidenceValidator] Running disproof engine...');

  const attempts: DisproofAttempt[] = [];
  const results: DisproofResult[] = [];

  const strategies: DisproofStrategy[] = [
  ...
  for (const finding of findings) {
    for (const strategy of strategies) {
      const attempt = executeDisproofStrategy(
        strategy,
        finding,
        contracts,
        functionRegistry,
      ...
      if (attempt.successful) {
        ...
        const newClass = determineNewClassificationAfterDisproof(
  ...
  // Compute summary
  const summary: DisproofSummary = {
    totalAttempts: attempts.length,
    successfulDisproofs: results.filter(r => 
      r.newClassification === 'false-positive' || r.newClassification === 'by-design'
    ).length,
    ...
    stillUncertain: results.filter(r => 
      r.newClassification === 'potential-bug' || r.newClassification === 'insufficient-evidence'
    ).length,
...
function executeDisproofStrategy(
  strategy: DisproofStrategy,
  finding: ClassifiedFinding,
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  ...
  switch (strategy) {
    case 'hidden-validation-check':
      ...
      if (func) {
        ...
        const hasModifierWithCheck = func.modifiers.some(m => 
          /require|check|validate|ensure/.test(m)
        );
    ...
    case 'implicit-invariant':
      ...
      const relevantInvariants = invariants.filter(inv =>
        inv.relatedStateVars?.some(v => 
          finding.location.variable?.includes(v) || v.includes(finding.location.variable?.split('.')[0] || '')
        ) ||
    ...
    case 'freshness-check':
      ...
      if (func && func.stateVariablesRead?.length > 0) {
        ...
        const readsTimestamp = func.stateVariablesRead.some(v => 
          /timestamp|lastUpdate|blockTime|lastBlock/i.test(v)
        );
    ...
    case 'access-control-verification':
      ...
      if (func) {
        ...
        const acLayers = func.modifiers.filter(m => 
          /only|require|check|auth|role|permission|pausable/i.test(m)
        ).length;
    ...
    case 'protocol-level-guarantee':
      ...
      const hasProxy = contracts.some(c => 
      ...
      const hasTimelock = contracts.some(c =>
        c.functions.some(f => /timelock|delay|schedule|queue/i.test(f.name))
      );
      const hasPauser = contracts.some(c =>
        c.functions.some(f => /pause|unpause/i.test(f.name))
      );
    ...
    case 'boundary-condition':
      ...
      if (func) {
        const hasBoundsChecks = func.stateVariablesRead?.some(v => 
          />=|<=|>|<|=/.test(findingsomeRelatedPattern(contracts, v)) || ''
        ) || func.calls?.some(c => /min|max|clamp|bounds/i.test(c));
...
function determineNewClassificationAfterDisproof(
  original: FindingClassification,
  strategy: DisproofStrategy,
  attempt: DisproofAttempt
): FindingClassification {
  // Strong disproof with certain strategies leads to false positive or by-design
  const strongDisproofStrategies: DisproofStrategy[] = [
    'hidden-validation-check',
    'access-control-verification',
    'economic-analysis'
  ];

  const mediumDisproofStrategies: DisproofStrategy[] = [
    'implicit-invariant',
    'rollback-behavior',
    'protocol-level-guarantee'
  ];
...
function generateDisproofReasoning(
  strategy: DisproofStrategy,
  attempt: DisproofAttempt,
  finding: ClassifiedFinding
): string {
...
function calculateDisproofConfidence(_strategy: DisproofStrategy, attempt: DisproofAttempt): number {
...
function findAllFunctionsCalling(contracts: SolidityContract[], pattern: RegExp): FunctionDef[] {
...
function findingsomeRelatedPattern(contracts: SolidityContract[], varName: string): string {
  // Simplified - just return the variable name itself
  return varName;
}

function assessCapitalRequirement(_finding: ClassifiedFinding): string {
...
function assessProfitPotential(finding: ClassifiedFinding): string {
...
function countStrategiesBySuccess(results: DisproofResult[]): Record<string, number> {
...
function assessAllConfidence(
  registry: ClassificationRegistry,
  reachability: ReachabilityAnalysisReport,
  disproof: DisproofReport,
  verbose: boolean = false
...
function assessSingleConfidence(
  finding: ClassifiedFinding,
  reachability: ReachabilityAnalysisReport,
  disproof: DisproofReport
): ConfidenceAssessment {
  ...
  const overallConfidence = Math.round(
    evidenceScore.score * weights.evidence +
...
function calculateEvidenceScore(finding: ClassifiedFinding): ComponentScore {
  ...
  const codeEvidence = finding.supportingEvidence.filter(e => 
    e.itemType === 'code-pattern' || e.itemType === 'execution-path'
  ).length;
  factors.push({
...
function calculateReachabilityScore(finding: ClassifiedFinding, reachability: ReachabilityAnalysisReport): ComponentScore {
  const factors: ScoreFactor[] = [];
...
function calculateImpactScore(finding: ClassifiedFinding): ComponentScore {
  const factors: ScoreFactor[] = [];
...
function calculateConsistencyScore(finding: ClassifiedFinding): ComponentScore {
  const factors: ScoreFactor[] = [];
  ...
  const expectedPatterns: Record<FindingClassification, string[]> = {
...
function generateNextSteps(rec: RecommendedAction, finding: ClassifiedFinding): string[] {
  switch (rec) {
    case 'immediate-fix':
      return [
...
function generateFinalVerdict(
  ...
  for (const finding of registry.entries) {
    ...
    let finalVerdict: FinalVerdict = 'cannot-determine';
    let recAction: RecommendedAction = 'defer';
...
function mapToFinalVerdict(classification: FindingClassification, confidence: number, isReachable?: boolean): FinalVerdict {
...
function computeVerdictSummary(verdicts: VerdictEntry[]): VerdictSummary {
  const counts = {
  ...
  for (const v of verdicts) {
    switch (v.finalVerdict) {
      case 'confirmed-vulnerability': counts.confirmedVulns++; break;
      case 'potential-vulnerability': counts.potentialVulns++; break;
      case 'false-positive': counts.falsePositives++; break;
      case 'by-design': counts.byDesign++; break;
      case 'cannot-determine': counts.cannotDetermine++; break;
...
function findRegFunc(registry?: Map<string, RegisteredFunction[]>, contract?: string, funcName?: string): RegisteredFunction | undefined {
...
function generateProofRequirements(
  ...
  const result: ProofRequirements[] = [];

  for (const finding of findings) {
    const path = reachability.paths.find(p => p.findingId === finding.findingId);
    
    if (!path) continue;  // Skip unreachable findings for now
    ...
    reqs.push({
      ...
      ...(stateSatisfied ? {} : { evidence: { itemId: `REQ3_EVID_${finding.findingId}`, itemType: 'state-prerequisite' as any, description: path.reachabilityReason, strength: 'strong' as any, isSupporting: true } })
    });
    ...
    for (const r of reqs) r.findingId = finding.findingId;
    ...
    const overallStatus: ProofRequirements['overallStatus'] = 
      metCount >= 8 ? 'proven-reachable' :
...
function compileValidationSummary(
  ...
  disproof: DisproofReport,
  confidence: ConfidenceAssessment[],
  verdict: FinalVerdictTable
): ValidationSummary {
  const recommendations: ValidationRecommendation[] = [];

  // Critical and confirmed → immediate fix
  const criticalConfirmed = verdict.verdicts.filter(v => 
    v.finalVerdict === 'confirmed-vulnerability' || 
  ...
  const dist: Record<FindingClassification, number> = {
  ...
  return {
    ...
    mostCommonCategory: Object.keys(registry.statistics.byCategory).sort((a, b) => 
      (registry.statistics.byCategory[b] || 0) - (registry.statistics.byCategory[a] || 0))[0] || 'unknown',
    recommendations,
 ...
 */
export function exportValidationResult(result: EvidenceValidationResult): any {
  return {
 ...
 */
export function generateValidationMarkdown(result: EvidenceValidationResult): string {
  ...
  for (const [cls, count] of Object.entries(dist)) {
    let icon = '?';
    if (cls === 'reachable-bug') icon = 'red-circle';
    else if (cls === 'potential-bug') icon = 'yellow-circle';
    else if (cls === 'false-positive') icon = 'white-circle';
    else if (cls === 'by-design') icon = 'blue-circle';
  ...
  for (const v of topVerdicts) {
    let icon = '?';
    if (v.finalVerdict === 'confirmed-vulnerability') icon = 'red-circle';
    else if (v.finalVerdict === 'potential-vulnerability') icon = 'yellow-circle';
    else if (v.finalVerdict === 'false-positive') icon = 'white-circle';
    else if (v.finalVerdict === 'by-design') icon = 'blue-circle';
  ...
  if (result.summary.recommendations.length > 0) {
    ...
    for (const rec of result.summary.recommendations) {
      let icon = '[ ]';
      if (rec.priority === 'critical') icon = '[!!]';
      else if (rec.priority === 'high') icon = '[!]';
...
// - Cross-phase consensus measurement
// - Disproof survival rate integration  
// - Historical pattern matching
// - Weighted calibration (5 factors)
// ============================================================

// -----------------------------------------------------------
// CALIBRATED CONFIDENCE TYPES
// -----------------------------------------------------------

export interface CalibratedConfidenceResult {
  timestamp: string;
  calibratedAssessments: CalibratedConfidenceAssessment[];
  calibrationSummary: CalibrationSummary;
  redteamOptimizedOutput: RedteamOptimizedConfidence[];
}

export interface CalibratedConfidenceAssessment extends ConfidenceAssessment {
  // NEW: Multi-dimensional calibration factors
  calibrationFactors: CalibrationFactors;
  
  // NEW: Cross-phase intelligence
  crossPhaseConsensus: CrossPhaseConsensus;
  
  // NEW: Disproof survival analysis
  disproofSurvival: DisproofSurvivalAnalysis;
  
  // NEW: Historical pattern correlation
  historicalCorrelation: HistoricalPatternMatch;
  
  // NEW: Final calibrated score (may differ from original)
  calibratedConfidence: number;
  
  // NEW: Confidence stability indicator
  stability: 'stable' | 'volatile' | 'improving' | 'degrading';
  
  // NEW: Redteam consumption hints
  redteamHints: RedteamHint[];
}

export interface CalibrationFactors {
  /** Factor 1: Classification strength (from Evidence Validator) */
  classificationScore: {
    value: number;           // 0-100
  ...
  reachabilityCompleteness: {
    value: number;           // 0-100
    weight: number;          // Default: 0.25
    pathCoverage: number;     // % of possible paths analyzed
    prerequisiteSatisfaction: number;  // How many prerequisites are satisfiable
    rationale: string;
  };
  ...
  disproofSurvivalRate: {
    ...
    totalAttempts: number;
    strongestDisproofStrategy: string | null;
    rationale: string;
  };
  
  /** Factor 4: Cross-phase consensus */
  crossPhaseAgreement: {
    value: number;           // 0-100
    weight: number;          // Default: 0.15
    phasesAgreeing: number;
    totalPhases: number;
  ...
  historicalMatchScore: {
    ...
    historicalPrecedentConfidence: number;
    similarExploitCount: number;
    rationale: string;
  };
...
export interface CrossPhaseConsensus {
  findingId: string;
  phaseEvidence: {
    phase1?: PhaseEvidence;
    phase2?: PhaseEvidence;
    phase3?: PhaseEvidence;
    phase4?: PhaseEvidence;
  };
  consensusVerdict: 'confirmed-by-multiple' | 'single-source' | 'contradicted' | 'unknown';
...
export interface DisproofSurvivalAnalysis {
  survivedAllAttempts: boolean;
  survivalRate: number;        // 0-100
  weakestPoint: string;        // Where disproof almost succeeded
  residualRisk: 'none' | 'low' | 'medium' | 'high';
  recommendedAdditionalChecks: string[];
}

export interface HistoricalPatternMatch {
  patternType: HistoricalExploitPattern | null;
  confidenceBoost: number;     // +0 to +30 points
  penaltyApplied: number;      // 0 to -20 points
  similarKnownExploits: KnownExploitReference[];
  isNovelAttackPattern: boolean;
}

export type HistoricalExploitPattern =
  | 'reentrancy-classic'
  | 'reentrancy-cross-function'
  | 'oracle-manipulation-flash-loan'
  | 'oracle-manipulation-spoofing'
...
export interface RedteamHint {
  category: 'attack-vector' | 'prerequisite' | 'amplification' | 'detection-evasion' | 'profit-extraction';
  hint: string;
  priority: 'critical' | 'high' | 'medium';
  estimatedDifficulty: 'trivial' | 'easy' | 'moderate' | 'difficult';
}

export interface RedteamOptimizedConfidence {
  findingId: string;
  calibratedConfidence: number;
  originalConfidence: number;
  delta: number;                    // Change after calibration
  
  // Pre-computed for Redteam consumption
  shouldInvestigate: boolean;       // confidence >= 50%
  shouldReport: boolean;            // confidence >= 70%
  isHighValueTarget: boolean;       // confidence >= 80% AND high impact
  
  // Attack surface summary
  attackSurface: {
    entryPointsAccessible: boolean;
  ...
  // Quick reference
  keyTrackatorSources: string[];    // Which Trackator fields support this
  conflictingPhases: string[];      // Phases that disagree (if any)
  
  // Recommended Redteam action
  recommendedAction: 'build-poc' | 'investigate-further' | 'de-prioritize' | 'dismiss';
...
export interface CalibrationSummary {
  ...
  maxDelta: number;                 // Largest confidence change
  stableFindings: number;           // Stability == 'stable'
  volatileFindings: number;         // Stability != 'stable'
  highValueTargets: number;         // isHighValueTarget == true
  crossPhaseConfirmed: number;      // Consensus == 'confirmed-by-multiple'
  historicalMatches: number;        // Has known exploit pattern
  
  // Distribution after calibration
  confidenceDistribution: {
    critical90Plus: number;
    high70_89: number;
    medium50_69: number;
    lowBelow50: number;
  };
  ...
  calibrationQuality: {
    factorsApplied: boolean;
    crossPhaseAnalyzed: boolean;
    historicalPatternsChecked: boolean;
    disproofIntegrated: boolean;
  };
}
...
const HISTORICAL_EXPLOIT_PATTERNS: Record<string, {
  pattern: HistoricalExploitPattern;
  indicators: RegExp[];
  baseConfidence: number;
  knownExploits: KnownExploitReference[];
}> = {
  'reentrancy-classic': {
...
export function calibrateConfidence(
  ...
  if (verbose) console.log('[Calibration] Starting multi-dimensional confidence calibration...');
  
  const calibratedAssessments: CalibratedConfidenceAssessment[] = [];
  const redteamOutput: RedteamOptimizedConfidence[] = [];
  ...
  for (const assessment of assessments) {
    ...
    if (!finding) {
      if (verbose) console.log(`[Calibration] Warning: No registry entry for ${assessment.findingId}`);
      continue;
    }
    ...
    const calibratedConfidence = Math.round(
      factors.classificationScore.value * weights.classification +
    ...
    const calibrated: CalibratedConfidenceAssessment = {
      ...assessment,
    ...
    if (redteamOpt.isHighValueTarget) highValueCount++;
    if (crossPhase.consensusVerdict === 'confirmed-by-multiple') crossPhaseConfirmed++;
    if (historical.patternType !== null && historical.patternType !== 'unknown-pattern') historicalMatchCount++;
    
    // Update distribution
    if (calibratedConfidence >= 90) confidenceDist.critical90Plus++;
    else if (calibratedConfidence >= 70) confidenceDist.high70_89++;
    else if (calibratedConfidence >= 50) confidenceDist.medium50_69++;
    else confidenceDist.lowBelow50++;
  }
...
function calculateReachabilityCompleteness(path: ExecutionPath): number {
  let score = 40;  // Base score for having a path
  
  if (path.isReachable) score += 30;
  if (path.entryPoint.isPermissionless) score += 15;
  if (path.callChain.length <= 3) score += 10;  // Short paths are more complete
  if (path.statePrerequisites.every(p => p.howToSatisfy !== 'Unknown')) score += 5;
  
  return Math.min(score, 100);
}

function analyzeCrossPhaseConsensus(
  finding: ClassifiedFinding,
  options?: {
    phase1Findings?: StorageHighRiskFinding[];
    phase2Findings?: CouplingCriticalFinding[];
  ...
  const phaseEvidence: Record<number, PhaseEvidence> = {};
  ...
  let consensusVerdict: 'confirmed-by-multiple' | 'single-source' | 'contradicted' | 'unknown';
  let consensusLevel: 'strong' | 'moderate' | 'weak' | 'conflicting';
  ...
  return {
    findingId: finding.findingId,
    phaseEvidence: phaseEvidence as any,
    consensusVerdict,
    consensusStrength
  };
}

function analyzeDisproofSurvival(
  finding: ClassifiedFinding,
  disproof: DisproofReport
): DisproofSurvivalAnalysis {
  ...
  let residualRisk: 'none' | 'low' | 'medium' | 'high';
  if (survivalRate >= 90) residualRisk = 'none';
  else if (survivalRate >= 70) residualRisk = 'low';
  else if (survivalRate >= 50) residualRisk = 'medium';
  else residualRisk = 'high';
  ...
  if (successful.length > 0) {
    const mostConvincing = successful.sort((a, b) => 
      b.evidenceFound.filter(e => e.strength === 'strong').length - 
      a.evidenceFound.filter(e => e.strength === 'strong').length
    )[0];
...
function matchHistoricalPatterns(finding: ClassifiedFinding): HistoricalPatternMatch {
  const textToMatch = `${finding.title} ${finding.description} ${finding.category}`.toLowerCase();
  
  let bestMatch: { pattern: HistoricalExploitPattern; score: number } | null = null;
  let maxScore = 0;
  ...
  const isNovel = finding.supportingEvidence.some(e => 
    e.itemType === 'heuristic' && e.description.includes('novel') || 
...
function determineStability(
  ...
  _original: number,
  _calibrated: number
): 'stable' | 'volatile' | 'improving' | 'degrading' {
  if (delta <= 5) return 'stable';
  if (_calibrated > _original) return 'improving';
  return 'degrading';
}

function generateRedteamHints(
  finding: ClassifiedFinding,
  calibratedConfidence: number,
  crossPhase: CrossPhaseConsensus,
  historical: HistoricalPatternMatch
): RedteamHint[] {
  const hints: RedteamHint[] = [];
  ...
  if (historical.patternType && historical.patternType !== 'unknown-pattern') {
    ...
    if (historical.similarKnownExploits.length > 0) {
      const biggestExploit = historical.similarKnownExploits.sort((a, b) => 
        parseInt(b.lossUsd.replace(/[^0-9]/g, '')) - parseInt(a.lossUsd.replace(/[^0-9]/g, ''))
      )[0];
...
function buildRedteamOutput(
  calibrated: CalibratedConfidenceAssessment,
  finding: ClassifiedFinding,
  reachability: ReachabilityAnalysisReport
): RedteamOptimizedConfidence {
  const path = reachability.paths.find(p => p.findingId === finding.findingId);
  ...
  let recommendedAction: RedteamOptimizedConfidence['recommendedAction'];
  ...
  const profitMap: Record<string, string> = {
 ...
 */
export function getInvestigationQueue(
  calibrated: CalibratedConfidenceResult
): RedteamOptimizedConfidence[] {
 ...
 */
export function exportCalibratedResults(result: CalibratedConfidenceResult): any {
  return {
    timestamp: result.timestamp,
