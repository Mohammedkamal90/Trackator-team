...
export interface StateDiffOptions {
  baselineTrace?: FoundryTrace;
  inventories?: Map<string, StateInventory>;
  alertRules?: any[];
  verbose?: boolean;
}
...
export function computeStateDiffs(
  ...
  for (const step of trace.trace) {
    if (processedAddresses.has(step.address)) continue;
 ...
 */
function computeSingleContractDiff(
  step: TraceStep,
  _trace: FoundryTrace,
  ...
  if (step.storageAfter) {
    for (const [slotHex, value] of Object.entries(step.storageAfter.slots)) {
      ...
      const change: SlotChange = {
        slot: slotHex,
        afterValue: value as string,
        beforeValue,
        changeType: determineChangeType(beforeValue, value as string),
        deviation: calculateDeviation(beforeValue, value as string)
      };
      ...
      if (inventory) {
        ...
        if (varInfo) {
          ...
          try {
            ...
            change.decodedAfter = decodeStorageValue(value as string, varInfo.typeInfo, varInfo.offset);
          } catch (e) {
 ...
 */
function determineChangeType(before: string, after: string): SlotChange['changeType'] {
 ...
 */
function calculateDeviation(before: string, after: string): number | undefined {
  try {
 ...
 */
function computeBalanceChange(step: TraceStep): BalanceChange {
  ...
  const tokenChanges: TokenBalanceChange[] = extractTokenTransfers(step);
 ...
 */
function extractTokenTransfers(step: TraceStep): TokenBalanceChange[] {
  const transfers: TokenBalanceChange[] = [];
 ...
 */
function findVariableBySlot(inventory: StateInventory, slot: number): any {
 ...
 */
function decodeStorageValue(hexValue: string, typeInfo: any, offset?: number): any {
 ...
 */
function checkForAnomalies(diffs: StateDiff[], alertRules: any[]): void {
...
function generateAnomalyMessage(rule: any, slotChange: SlotChange): string {
...
export function compareTraces(
  baseline: FoundryTrace,
  current: FoundryTrace,
  options: StateDiffOptions = {}
): StateDiff[] {
  ...
  const differentialDiffs: StateDiff[] = [];
  ...
  for (const currentDiff of currentDiffs) {
    ...
    const newSlotChanges = currentDiff.slotChanges.filter(currentSlot => {
      ...
      if (!baselineSlot) return true; // New slot changed
 ...
 */
export function generateStateDiffSummary(diffs: StateDiff[]): {
  totalContractsTouched: number;
  totalSlotsChanged: number;
  significantChanges: number;
  anomaliesFound: number;
  contractsByChangeCount: { contract: string; changes: number }[];
} {
  let totalSlotsChanged = 0;
  let significantChanges = 0;
  let anomaliesFound = 0;
  const contractsByChangeCount: { contract: string; changes: number }[] = [];
