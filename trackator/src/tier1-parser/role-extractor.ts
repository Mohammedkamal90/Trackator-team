...
import { buildFunctionRegistry, RegisteredFunction as RF } from './function-registry';

/**
 * Main entry point for role extraction
 */
export function extractProtocolRoles(
  contracts: SolidityContract[],
  registry?: Map<string, RF[]>
): RoleExtractionResult {
  
  const funcRegistry = registry || buildFunctionRegistry(contracts);
...
function extractTrustedRoles(
  contracts: SolidityContract[],
  registry: Map<string, RF[]>
): ProtocolRole[] {
  
  const roles: ProtocolRole[] = [];
  const seenRoles = new Map<string, ProtocolRole>(); // Deduplication
  
  for (const contract of contracts) {
    // Skip interfaces and abstract contracts for role definition
    if (contract.abstract || isInterface(contract)) continue;
 ...
 */
function detectOwnerRole(contract: SolidityContract): ProtocolRole | null {
  // Look for owner-related patterns
  const hasOwnerVar = contract.stateVariables.some(sv => 
    sv.name.toLowerCase() === 'owner' ||
    sv.name.toLowerCase() === '_owner' ||
    sv.name.toLowerCase() === 'admin'
  );
  
  const hasOwnerFunc = contract.functions.some(f =>
    f.name.toLowerCase() === 'owner' &&
    f.stateMutability === 'view'
  );
  
  const hasOnlyOwnerModifier = contract.functions.some(f =>
    f.modifiers.some(m => m.toLowerCase().includes('onlyowner'))
  );
  
  const hasTransferOwnership = contract.functions.some(f =>
  ...
  let addressType: ProtocolRole['addressType'] = 'unknown';
  
  const ownerVar = contract.stateVariables.find(sv =>
    sv.name.toLowerCase() === 'owner' || sv.name.toLowerCase() === '_owner'
  );
  if (ownerVar) {
  ...
  const capabilities = findCapabilitiesForModifier(
    contract, 
    'onlyOwner', 
 ...
 */
function detectAdminRoles(contract: SolidityContract): ProtocolRole[] {
  const roles: ProtocolRole[] = [];
  
  // Look for admin state variables
  const adminVars = contract.stateVariables.filter(sv =>
    sv.name.toLowerCase().includes('admin') && 
  ...
  for (const adminVar of adminVars) {
    ...
    const capabilities = findCapabilitiesForModifier(
      contract,
      modifierName,
      [adminVar.name.toLowerCase(), `only${adminVar.name.toLowerCase()}`]
    );
...
function detectSpecialRoles(
  ...
  const specialPatterns = [
    { keywords: ['guardian', 'pause'], name: 'Guardian', trustLevel: 'HIGH' as TrustLevel },
    { keywords: ['manager', 'portfolio'], name: 'Manager', trustLevel: 'MEDIUM' as TrustLevel },
    { keywords: ['operator'], name: 'Operator', trustLevel: 'MEDIUM' as TrustLevel },
    { keywords: ['liquidator'], name: 'Liquidator', trustLevel: 'LOW' as TrustLevel },
    { keywords: ['servicer'], name: 'Servicer', trustLevel: 'MEDIUM' as TrustLevel },
    { keywords: ['originator'], name: 'Originator', trustLevel: 'MEDIUM' as TrustLevel },
    { keywords: ['whitelist'], name: 'Whitelisted User', trustLevel: 'LOW' as TrustLevel },
  ];
  ...
  for (const pattern of specialPatterns) {
    ...
    const restrictedFunctions = matchingFunctions.filter(f =>
 ...
 */
function detectGovernanceRoles(contract: SolidityContract): ProtocolRole[] {
  const roles: ProtocolRole[] = [];
  
  // Timelock detection
  const hasTimelock = contract.stateVariables.some(sv =>
    sv.name.toLowerCase().includes('timelock') ||
    sv.name.toLowerCase().includes('delay')
  ) || contract.functions.some(f =>
  ...
  if (hasTimelock) {
    const timelockFuncs = contract.functions.filter(f =>
      f.name.toLowerCase().includes('timelock') ||
  ...
  // Multisig detection
  const hasMultisig = contract.stateVariables.some(sv =>
    sv.name.toLowerCase().includes('guardians') ||
...
function extractNonTrustedRoles(
  contracts: SolidityContract[],
  registry: Map<string, RF[]>
): ProtocolRole[] {
  
  const roles: ProtocolRole[] = [];
  const seenNonTrusted = new Set<string>();
  ...
  for (const contract of contracts) {
    ...
    const publicFunctions = contract.functions.filter(f => {
      if (f.visibility !== 'external' && f.visibility !== 'public') return false;
      if (f.stateMutability === 'view' || f.stateMutability === 'pure') return false;
      
      // Skip constructors
      if (f.kind === 'constructor') return false;
...
function detectBorrowerRole(
  ...
  if (borrowRelatedFuncs.length === 0) return null;
...
function findCapabilitiesForModifier(
  contract: SolidityContract,
  primaryModifier: string,
  modifierVariants: string[]
): RoleCapability[] {
  ...
  for (const func of contract.functions) {
    ...
    const usesModifier = modifierVariants.some(variant => 
      funcModifiers.some(fm => fm.includes(variant))
    );
...
function createCapabilityFromFunction(func: FunctionDef, contractName: string): RoleCapability {
...
function categorizeCapability(func: FunctionDef): RoleCapability['category'] {
  const nameLower = func.name.toLowerCase();
...
function assessCapabilityImpact(func: FunctionDef): RoleCapability['impact'] {
  const body = func.body as any;
  const nameLower = func.name.toLowerCase();
...
function generateCapabilityDescription(func: FunctionDef): string {
  const nameLower = func.name.toLowerCase();
  const params = func.parameters.map(p => p.type).join(', ');
  
  // Generate human-readable descriptions
  if (nameLower.includes('deposit')) return `Deposit assets into the protocol`;
  if (nameLower.includes('withdraw')) return `Withdraw assets from the protocol`;
  if (nameLower.includes('transfer')) return `Transfer tokens/assets`;
  if (nameLower.includes('mint')) return `Mint new tokens/shares`;
  if (nameLower.includes('burn')) return `Burn existing tokens/shares`;
  if (nameLower.includes('pause')) return `Pause protocol operations`;
  if (nameLower.includes('unpause')) return `Resume paused operations`;
  if (nameLower.includes('grant')) return `Grant permissions to an address`;
  if (nameLower.includes('revoke')) return `Revoke permissions from an address`;
  if (nameLower.includes('set') && nameLower.includes('oracle')) return `Update oracle price feed`;
  if (nameLower.includes('set') && nameLower.includes('fee')) return `Update fee configuration`;
  if (nameLower.includes('fund')) return `Fund loans or provide capital`;
  if (nameLower.includes('collect')) return `Collect payments or revenue`;
  if (nameLower.includes('liquidate')) return `Liquidate unhealthy positions`;
  if (nameLower.includes('borrow')) return `Borrow against collateral`;
  if (nameLower.includes('repay')) return `Repay outstanding debt`;
  if (nameLower.includes('emergency')) return `Emergency extraction of funds`;
  
  return `Execute ${func.name}(${params})`;
}

function deriveConstraints(capabilities: RoleCapability[]): string[] {
  ...
  const hasTimelocked = capabilities.some(c => 
    c.description.toLowerCase().includes('delay') ||
...
function calculateTrustLevel(capabilities: RoleCapability[]): TrustLevel {
...
function generateTrustReasoning(
  roleName: string,
...
function assessOwnerRisk(capabilities: RoleCapability[]): string {
  const criticalCaps = capabilities.filter(c => c.impact === 'critical');
...
function assessRoleRisk(roleName: string, capabilities: RoleCapability[]): string {
...
function assessPublicRisk(roleName: string, capabilities: RoleCapability[]): string {
  const financialCaps = capabilities.filter(c => c.category === 'financial');
...
function generateRoleSummary(
  ...
  const hasTimelock = contracts.some(c =>
    c.stateVariables.some(sv => sv.name.toLowerCase().includes('timelock'))
  );
  
  const hasMultisig = contracts.some(c =>
    c.stateVariables.some(sv => 
      sv.name.toLowerCase().includes('guardians') ||
      (sv.name.toLowerCase().includes('owners') && sv.type.includes('[]')) ||
      sv.name.toLowerCase().includes('threshold')
    )
  ...
  return {
    ...
    highTrustCount: trustedRoles.filter(r => 
      r.trustLevel === 'CRITICAL' || r.trustLevel === 'HIGH'
    ).length,
...
function isInterface(contract: SolidityContract): boolean {
...
function extractRoleNameFromModifier(
  modifier: string,
  func: FunctionDef,
  contract: SolidityContract
): string | null {
  ...
  if (funcNameLower.includes('manager')) return 'MANAGER';
  if (funcNameLower.includes('guardian')) return 'GUARDIAN';
  if (funcNameLower.includes('operator')) return 'OPERATOR';
  
  return null;
}

function findRoleConstant(roleName: string, contract: SolidityContract): string | undefined {
  ...
  const candidates = contract.stateVariables.filter(sv => {
    ...
    return (
      (sv.type.includes('bytes32') || sv.type.includes('roles')) &&
...
function inferTargetRole(func: FunctionDef, trustedRoles: ProtocolRole[]): string | null {
  const nameLower = func.name.toLowerCase();
...
function categorizePublicFunctions(
  functions: FunctionDef[],
  contract: SolidityContract
): Record<string, FunctionDef[]> {
  
  const categories: Record<string, FunctionDef[]> = {
    'User (General)': [],
    'Depositor': [],
    'Trader': [],
    'Liquidator': []
...
export {
  ...
  detectRBACRoles,
  detectSpecialRoles,
  detectGovernanceRoles,
  extractNonTrustedRoles,
  findCapabilitiesForModifier,
  createCapabilityFromFunction,
  categorizeCapability,
  assessCapabilityImpact
};
