...
import {
  SolidityContract,
  StateInventory,
  RegisteredFunction as TypesRegisteredFunction,
  CallGraph as TypesCallGraph,
  MermaidDiagram,
  FunctionDef,
  EventDef,
  ModifierDef
} from '../types';
import { generateStateInventory } from './state-inventory';
import { buildFunctionRegistry, getEntryPoints, getHighRiskFunctions, RegisteredFunction } from './function-registry';
import { buildCallGraph, generateMermaidCallGraph, generateSequenceDiagram, generateClassDiagram, CallGraph } from './call-graph';

// Re-export for backward compatibility
export type { RegisteredFunction };

export interface DiagramSet {
  contractDiagrams: MermaidDiagram[];
  stateDiagrams: MermaidDiagram[];
  flowDiagrams: MermaidDiagram[];
  riskDiagrams: MermaidDiagram[];
  valueFlowDiagrams: MermaidDiagram[];  // NEW: Value/Money flow diagrams (Mermaid)
  valueTransfers: ValueTransfer[];      // NEW: Raw transfer data for summary tables
}

/**
 * Interface for detected value transfer patterns
 */
export interface ValueTransfer {
  from: string;        // Source (contract, "User", "External")
  to: string;          // Destination (contract, "User", "External")
  asset: string;       // What's being transferred ("ETH", "ERC20", "NFT", "Shares", etc.)
  viaFunction: string; // Function that triggers this transfer
  contractName: string;// Contract containing the function
  type: 'transfer' | 'mint' | 'burn' | 'approve' | 'deposit' | 'withdraw' | 'swap' | 'pay' | 'fund' | 'collect';
  condition?: string; // Any conditions on this transfer
}
...
export function generateAllDiagrams(
  ...
  options: { includeRisk?: boolean; includeState?: boolean; includeFlows?: boolean; includeValueFlow?: boolean } = {}
): DiagramSet {
  const {
    includeRisk = true,
    includeState = true,
    includeFlows = true,
    includeValueFlow = true  // NEW: Include value flow by default
  } = options;
  ...
  const registry = buildFunctionRegistry(contracts);
  
  // Contract structure diagrams
  const classDiagramStr = generateClassDiagram(contracts);
  diagramSet.contractDiagrams.push(
    generateContractOverview(contracts),
    { type: 'classDiagram' as const, title: 'Class Diagram', code: classDiagramStr, description: 'Class inheritance and relationships' },
    generateInheritanceGraph(contracts)
  );
  
  // Per-contract diagrams
  for (const contract of contracts) {
    diagramSet.contractDiagrams.push(
      generateContractDetail(contract),
      generateEventFlow(contract)
    );
    
    if (includeState) {
      const inventory = generateStateInventory(contract);
      const regFuncs = registry.get(contract.name) || [];
      diagramSet.stateDiagrams.push(
        generateStorageLayout(inventory),
        generateStateTransition(contract, regFuncs)
      );
    }
    
    if (includeFlows) {
      diagramSet.flowDiagrams.push(
        ...generateExecutionFlows(contract, callGraph as any, registry)
      );
    }
    
    if (includeRisk) {
      const regFuncs = registry.get(contract.name) || [];
      diagramSet.riskDiagrams.push(
        generateRiskHeatmap(contract, regFuncs),
        generateAccessControlMap(contract, regFuncs)
      );
    }
  }
  ...
  if (includeValueFlow) {
    ...
    diagramSet.valueFlowDiagrams.push(
      ...generateProtocolValueFlows(contracts, registry)
    );
  }
 ...
 */
function generateContractOverview(contracts: SolidityContract[]): MermaidDiagram {
 ...
 */
function generateInheritanceGraph(contracts: SolidityContract[]): MermaidDiagram {
  ...
  for (const contract of contracts) {
    if (processed.has(contract.name)) continue;
...
function getInheritanceChain(name: string, contracts: SolidityContract[]): string[] {
 ...
 */
function generateContractDetail(contract: SolidityContract): MermaidDiagram {
  ...
  for (const event of contract.events) {
    const params = event.parameters.map(p =>
      p.indexed ? `${p.type} indexed` : p.type
    ).join(', ');
 ...
 */
function generateStorageLayout(inventory: any): MermaidDiagram {
...
function generateStateTransition(
  ...
  registeredFuncs: RegisteredFunction[]
): MermaidDiagram {
  ...
  const stateChangingFuncs = registeredFuncs.filter((f: any) => {
 ...
 */
function generateEventFlow(contract: SolidityContract): MermaidDiagram {
 ...
 */
function generateAccessControlMap(
  contract: SolidityContract,
  registeredFuncs: RegisteredFunction[]
): MermaidDiagram {
  const lines: string[] = ['graph LR'];
  ...
  for (const func of registeredFuncs as any[]) {
 ...
 */
function sanitizeMermaidString(str: string): string {
  return str
    .replace(/\n/g, ' ')           // Remove newlines
...
function detectValueTransfers(
  ...
  for (const contract of contracts) {
    ...
    const hasERC20 = contract.stateVariables.some(sv => 
      sv.type.toLowerCase().includes('erc20') || 
      sv.type.toLowerCase().includes('ierc20') ||
      sv.name.toLowerCase().includes('token')
    );
    
    const hasETH = contract.functions.some(f => 
      f.stateMutability === 'payable'
    );
    
    const hasNFT = contract.stateVariables.some(sv =>
      sv.type.toLowerCase().includes('erc721') ||
 ...
 */
function generateProtocolValueFlows(
  contracts: SolidityContract[],
  registry: Map<string, RegisteredFunction[]>
): MermaidDiagram[] {
  const diagrams: MermaidDiagram[] = [];
  ...
  const contractsWithValueMovement = new Set(
    transfers.map(t => t.contractName)
  );
 ...
 */
function generateMainValueFlowDiagram(
  ...
  const drawnFlows = new Set<string>();
  
  for (const transfer of transfers) {
    const flowKey = `${transfer.from}->${transfer.to}:${transfer.asset}`;
    if (drawnFlows.has(flowKey)) continue;
 ...
 */
function generateDepositWithdrawalCycle(transfers: ValueTransfer[]): MermaidDiagram {
  const lines: string[] = ['sequenceDiagram'];
 ...
 */
function generateSecondaryMarketFlow(transfers: ValueTransfer[]): MermaidDiagram {
  const lines: string[] = ['sequenceDiagram'];
  
  const exchangeTransfers = transfers.filter(t => 
    t.contractName.toLowerCase().includes('exchange') ||
 ...
 */
function generateContractValueFlow(
  contractName: string, 
  transfers: ValueTransfer[]
): MermaidDiagram {
  const lines: string[] = ['graph TD'];
 ...
 */
export function getValueTransfers(
  contracts: SolidityContract[],
  registry?: Map<string, RegisteredFunction[]>
): ValueTransfer[] {
  const reg = registry || buildFunctionRegistry(contracts);
...
function getMutabilityBadge(mutability: string): string {
  switch (mutability) {
