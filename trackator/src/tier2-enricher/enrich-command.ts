...
export async function runEnrich(
  ...
  console.log(chalk.magenta.bold('║         TIER 2: AI ENRICHMENT                  ║'));
  console.log(chalk.magenta.bold('╚══════════════════════════════════════════════╝'));
  console.log('');
  ...
  const enrichmentResult = runEnrichment({
  ...
  console.log('');
  console.log(boxen(
    chalk.green.bold('✓ Enrichment Complete\n') +
    chalk.white('Protocol Type: ' + enrichmentResult.xray.protocolType.toUpperCase() + '\n') +
    chalk.white('Invariants Generated: ' + enrichmentResult.invariants.length + '\n') +
    chalk.white('Alert Rules Created: ' + enrichmentResult.alertRules.length + '\n') +
 ...
 */
function printTerminalEnrichment(result: any, verbose: boolean = false): void {
  console.log('\n');
  console.log(chalk.white.bold('═'.repeat(60)));
  console.log(chalk.white.bold('           ENRICHMENT RESULTS'));
  console.log(chalk.white.bold('═'.repeat(60)));
  console.log('');
  
  // Protocol Classification
  console.log(chalk.cyan.bold('┌─────────────────────────────────────────────────────────────┐'));
  console.log(chalk.cyan.bold('│              PROTOCOL CLASSIFICATION                     │'));
  console.log(chalk.cyan.bold('├─────────────────────────────────────────────────────────────┤'));
  console.log(chalk.white('  Type: ' + chalk.bold(result.xray.protocolType.toUpperCase())));
  console.log(chalk.white('  Confidence: Based on code pattern analysis'));
  console.log(chalk.cyan.bold('└─────────────────────────────────────────────────────────────┘'));
  console.log('');
  
  // Threat Model Summary
  console.log(chalk.yellow.bold('┌─────────────────────────────────────────────────────────────┐'));
  console.log(chalk.yellow.bold('│                   THREAT MODEL                            │'));
  console.log(chalk.yellow.bold('├─────────────────────────────────────────────────────────────┤'));
  
  const threatModel = result.xray.threatModel;
  console.log(chalk.white('  Assets at Risk: ' + threatModel.assetsAtRisk.length));
  console.log(chalk.white('  Entry Points:  ' + threatModel.entryPoints.length));
  console.log(chalk.white('  Adversaries:   ' + result.xray.adversaryProfiles.length));
  console.log(chalk.white('  Attack Vectors: ' + result.xray.attackVectors.length));
  
  if (threatModel.assetsAtRisk.length > 0) {
    console.log(chalk.gray('\n  Key Assets:'));
    for (const asset of threatModel.assetsAtRisk.slice(0, 5)) {
      console.log(chalk.gray('    • ' + asset.type + ': ' + asset.name + ' (' + asset.location + ')'));
    }
  }
  console.log(chalk.yellow.bold('└─────────────────────────────────────────────────────────────┘'));
  console.log('');
  ...
  console.log(chalk.green.bold('│                    INVARIANTS                              │'));
  console.log(chalk.green.bold('├─────────────────────────────────────────────────────────────┤'));
  ...
  for (const [category, invs] of Object.entries(invariantCategories)) {
    ...
    console.log(chalk.white('  ' + icon + ' ' + category + ': ' + invs.length + ' invariants'));
    
    if (verbose) {
      for (const inv of invs.slice(0, 3)) {
        const severityColor = getSeverityColor(inv.severity);
        console.log(chalk.gray('      - [' + severityColor(inv.severity) + '] ' + inv.template.substring(0, 60) + '...'));
      }
    }
  }
  console.log(chalk.green.bold('└─────────────────────────────────────────────────────────────┘'));
  ...
  console.log(chalk.red.bold('│                   ALERT RULES                              │'));
  console.log(chalk.red.bold('├─────────────────────────────────────────────────────────────┤'));
  ...
  for (const severity of severityOrder) {
    ...
    if (rules.length > 0) {
      ...
      if (rules.length > 3) {
        console.log(chalk.gray('      ... and ' + (rules.length - 3) + ' more'));
      }
    }
  }
  console.log(chalk.red.bold('└─────────────────────────────────────────────────────────────┘'));
  ...
  if (result.xray.trustAssumptions.length > 0) {
    ...
    console.log(chalk.hex('#9b59b6').bold('│                TRUST ASSUMPTIONS                           │'));
    console.log(chalk.hex('#9b59b6').bold('├─────────────────────────────────────────────────────────────┤'));
    ...
    for (const ta of result.xray.trustAssumptions.slice(0, 5)) {
      ...
      console.log(chalk.white('  ' + confidenceIcon + ' [' + ta.category + '] ' + ta.assumption));
      console.log(chalk.gray('     If violated: ' + ta.ifViolated));
    }
    console.log(chalk.hex('#9b59b6').bold('└─────────────────────────────────────────────────────────────┘'));
    console.log('');
  }
...
async function generateMarkdownReport(
  ...
  md += '| Critical Alerts | ' + result.alertRules.filter((r: any) => r.severity === 'critical').length + ' |\n';
  md += '| High Alerts | ' + result.alertRules.filter((r: any) => r.severity === 'high').length + ' |\n';
  ...
  md += '|-----------|----------|-----------|------------|\n';
  for (const rule of result.alertRules.filter((r: any) => r.severity === 'critical' || r.severity === 'high')) {
...
function getSeverityColor(severity: string): (text: string) => string {
  const colors: Record<string, (text: string) => string> = {
    'critical': chalk.red,
    'high': chalk.hex('#ff6b6b'),
    'medium': chalk.hex('#ffa726'),
    'low': chalk.hex('#ffee58'),
    'info': chalk.blue
  };
