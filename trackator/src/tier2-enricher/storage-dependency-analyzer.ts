...
// ============================================================
// CORE TYPE DEFINITIONS FOR STORAGE DEPENDENCY ANALYSIS
// ============================================================

export interface StorageWriteGraph {
  /** Variable name → list of functions that write to it */
  variableWriters: Map<string, VariableWriterInfo>;
  /** Functions that write multiple variables (complex state changes) */
  multiVariableWriters: MultiWriterInfo[];
  /** Variables written by multiple functions (contention points) */
  contentedVariables: ContendedVariable[];
  /** Summary statistics */
  summary: StorageDependencySummary;
}

export interface VariableWriterInfo {
  variableName: string;
  variableType: string;
  contract: string;
  slot?: number;
  writers: FunctionWriteInfo[];
  readerFunctions: FunctionReadInfo[];  // Functions that read this variable
  isValueBearing: boolean;             // Holds user funds/value
  accountingCategory?: AccountingCategory;
  relatedInvariants: string[];         // Invariant IDs that involve this variable
}

export interface FunctionWriteInfo {
  functionName: string;
  contract: string;
  visibility: string;
  accessControl: AccessControlLevel;
  isPermissionless: boolean;
  writeReason?: string;               // Why this function writes this variable
  accountingRule?: string;            // Which accounting rule this write belongs to
  ceiPattern: CEIPattern;             // CEI compliance for this function
  externalCallsBeforeWrite: string[]; // External calls that occur BEFORE this write
  externalCallsAfterWrite: string[];  // External calls that occur AFTER this write
  lineLocation?: number;
}

export interface FunctionReadInfo {
  functionName: string;
  contract: string;
  visibility: string;
  accessControl: AccessControlLevel;
  readPurpose?: string;               // Why this function reads this variable
  lineLocation?: number;
}

export interface MultiWriterInfo {
  functionName: string;
  contract: string;
  variablesWritten: string[];
  writeCount: number;
  hasMixedAccessControl: boolean;     // Writes to vars with different access levels
  riskLevel: 'critical' | 'high' | 'medium' | 'low';
}

export interface ContendedVariable {
  variableName: string;
  contract: string;
  writerCount: number;
  writerFunctions: string[];
  permissionlessWriterCount: number;
  hasCrossFunctionDependency: boolean; // Other functions depend on this var's state
  riskScore: number;                   // 0-100 based on contention + value
  synchronizationRequired: boolean;
}

export interface StorageDependencySummary {
  totalVariablesTracked: number;
  totalWriteOperations: number;
  totalReadOperations: number;
  contendedVariableCount: number;
  highRiskContentionCount: number;
  permissionlessWriteCount: number;
  valueBearingVariableCount: number;
  crossContractDependencyCount: number;
}

// ============================================================
// SHARED STATE MATRIX TYPES
// ============================================================

export interface SharedStateMatrix {
  /** Matrix rows = permissionless entry points */
  entryPoints: EntryPointRow[];
  /** Matrix columns = shared storage variables */
  sharedVariables: SharedVariableColumn[];
  /** Matrix cells [entryPoint][variable] */
  cells: Map<string, Map<string, MatrixCell>>;
  /** Risk summary */
  riskSummary: MatrixRiskSummary;
}
...
export interface EntryPointRow {
  ...
  access: 'permissionless' | 'restricted' | 'admin';
  variablesRead: string[];
  variablesWritten: string[];
  riskScore: number;
}

export interface SharedVariableColumn {
  variableName: string;
  contract: string;
  type: string;
  ...
  totalReaders: number;
  totalWriters: number;
  entryPointReaders: number;   // How many entry points read this
  entryPointWriters: number;   // How many entry points write this
  syncCategory?: SynchronizationCategory;
}

export type SynchronizationCategory =
  | 'accounting'          // Supply/balances consistency
  | 'authorization'       // Role/permission state
  | 'pricing'             // Price/oracle values
  | 'liquidity'           // Pool/liquidity state
  | 'solvency'            // Collateral/health factor
  | 'settlement'          // Pending/completed state
  | 'ownership'           // Owner/NFT tracking
  | 'timestamp'           // Time-related state
...
export interface MatrixCell {
  ...
  crossContractImpact?: string;    // Impact on other contracts
  invariantImpacted?: string[];    // Invariants that could be affected
}

export type DependencyType =
  | 'direct'              // Direct read/write
  | 'indirect-via-call'   // Via internal function call
  | 'indirect-via-storage' // Through shared storage with another function
  | 'cross-contract'      // Across contract boundaries
  | 'delegatecall'        // Via delegatecall context sharing;

export interface MatrixRiskSummary {
  highRiskCells: number;           // Cells with multiple risk factors
  criticalIntersections: number;    // Entry points touching value-bearing vars
  totalIntersectionPoints: number;
  averageRiskPerEntryPoint: number;
  weakestEntryPoint?: { name: string; riskScore: number };
  mostContendedVariable?: { name: string; contentionScore: number };
}

// ============================================================
// PER-WRITE EXPLANATION TYPES
// ============================================================

export interface WriteExplanationMap {
  /** Key: "contract.function.variable" → Explanation */
  explanations: Map<string, WriteExplanation>;
  /** Unexplained writes (need manual review) */
  unexplainedWrites: UnexplainedWrite[];
  /** Accounting rules discovered */
  discoveredRules: AccountingRule[];
}

export interface WriteExplanation {
  key: string;                       // "contract.function.variable"
  contract: string;
  function: string;
  variable: string;
  reason: WriteReasonCategory;
  explanation: string;               // Natural language explanation
  accountingRule?: string;           // Associated accounting rule
  invariantMaintained?: string;      // Invariant this write maintains
  invariantPotentiallyViolated?: string; // Invariant this could violate if buggy
  stateTransition?: StateTransitionDescription;
  precedenceOrder?: number;          // Order relative to other writes in function
  atomicityRequirement: AtomicityRequirement;
}

export type WriteReasonCategory =
  | 'initialize'          // Initial setup/construction
  | 'deposit'             // User deposit/mint
  | 'withdrawal'          // User withdrawal/burn
  | 'transfer'            // Transfer between users
  | 'balance-update'      // Balance adjustment
  | 'accrual'             // Interest/reward accrual
  | 'liquidation'         // Liquidation processing
  | 'price-update'        // Price/oracle update
  | 'approval'            // Allowance/approval change
  | 'ownership-change'    // Ownership transfer
  | 'state-flag'          // Boolean/state flag change
  | 'configuration'       // Parameter update
  | 'accounting'          // Accounting reconciliation
  | 'cache-update'        // Cached value refresh
...
export interface AccountingRule {
  ...
  functionsThatMaintain: string[];
  invariantsEnforced: string[];
  description: string;
}

export type AccountingCategory =
  | 'accounting'          // Supply/balances accounting consistency
  | 'conservation'        // Total supply must equal sum of balances
  | 'non-negative'        // Balances cannot go negative
  | 'bounding'           // Values within min/max bounds
  | 'monotonic'          // Only increases or only decreases
  | 'proportional'       // Ratio relationships
  | 'temporal-ordering'  // Timestamp/sequence ordering
  | 'access-control'     // Permission consistency
  | 'settlement-atomicity'; // All-or-nothing settlement
...
export type AtomicityRequirement =
  | 'atomic-single-slot'     // Single slot write (always atomic)
  | 'atomic-multi-slot'      // Multiple slots, must be atomic
  | 'cross-contract'         // Spans contracts
  | 'eventual'               // Eventually consistent OK
  | 'requires-reentrancy-guard'; // Vulnerable to reentrancy;
...
export interface TrustBoundaryMap {
  ...
  crossBoundaryFlows: CrossBoundaryFlow[];
  vulnerableBoundaries: VulnerableBoundary[];
}

export interface TrustBoundary {
  id: string;
  sourceContext: string;           // e.g., "ProtocolCore"
  targetContext: string;           // e.g., "ExternalOracle"
  boundaryType: BoundaryType;
  mechanism: string;               // External call, delegatecall, etc.
  ...
  dataExposed: string[];           // What data crosses this boundary
  stateModified: boolean;          // Whether state is modified across boundary
  ceiOrdering: 'safe' | 'unsafe' | 'unknown';
}

export type BoundaryType =
  | 'external-call'         // Regular external call
  | 'delegatecall'          // Delegatecall (full context sharing)
  | 'interface-call'        // Interface call (known target shape)
  | 'low-level-call'        // .call() .staticcall()
  | 'token-transfer'        // ERC20/721 transfer
  | 'oracle-read'           // Oracle price query
  | 'flash-loan'            // Flash loan interaction
  | 'callback'              // User-supplied callback
  | 'proxy-forward'         // Proxy call forwarding
  | 'library-call';         // Library call (internal but separate context)

export interface CrossBoundaryFlow {
  fromBoundary: string;
  toBoundary: string;
  dataType: 'value' | 'state-reference' | 'control-flow' | 'price' | 'permission';
  description: string;
  riskLevel: 'critical' | 'high' | 'medium' | 'low';
}

export interface VulnerableBoundary {
  boundaryId: string;
...
export interface StorageDependencyAnalysisOptions {
  contracts: SolidityContract[];
  functionRegistry?: Map<string, RegisteredFunction[]>;
  callEdges?: CallEdge[];
  invariants?: Invariant[];
  includeTrustBoundaries?: boolean;
  verbose?: boolean;
}

export interface StorageDependencyAnalysisResult {
  timestamp: string;
  storageWriteGraph: StorageWriteGraph;
  sharedStateMatrix: SharedStateMatrix;
  writeExplanations: WriteExplanationMap;
  trustBoundaryMap?: TrustBoundaryMap;
  // Quick access indexes
  highRiskFindings: HighRiskFinding[];
  recommendations: Recommendation[];
}

export interface HighRiskFinding {
  id: string;
  type: 'contention' | 'permissionless-write' | 'sync-risk' | 'trust-boundary' | 'cei-violation';
  severity: 'critical' | 'high' | 'medium' | 'low';
  title: string;
  description: string;
 ...
 */
export function analyzeStorageDependencies(
  options: StorageDependencyAnalysisOptions
): StorageDependencyAnalysisResult {
  const {
    contracts,
    functionRegistry,
    callEdges = [],
  ...
  if (verbose) console.log('[StorageDepAnalyzer] Starting comprehensive storage dependency analysis...');

  // Step 1: Build Storage Write Graph
  const storageWriteGraph = buildStorageWriteGraph(
    contracts,
    functionRegistry,
    invariants,
    verbose
  ...
  const sharedStateMatrix = buildSharedStateMatrix(
    contracts,
    functionRegistry,
    storageWriteGraph,
    callEdges,
  ...
  const writeExplanations = generateWriteExplanations(
    storageWriteGraph,
    invariants,
    verbose
  );
  ...
  let trustBoundaryMap: TrustBoundaryMap | undefined;
  if (includeTrustBoundaries) {
    trustBoundaryMap = buildTrustBoundaryMap(
      contracts,
      functionRegistry,
      callEdges,
      verbose
  ...
  const highRiskFindings = compileHighRiskFindings(
    storageWriteGraph,
    sharedStateMatrix,
    trustBoundaryMap,
    verbose
...
function buildStorageWriteGraph(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  invariants: Invariant[] = [],
  verbose: boolean = false
): StorageWriteGraph {
  if (verbose) console.log('[StorageDepAnalyzer] Building storage write graph...');

  const variableWriters = new Map<string, VariableWriterInfo>();
  const multiWriterList: MultiWriterInfo[] = [];
  const contendedVars: ContendedVariable[] = [];
  ...
  for (const [, varInfo] of variableWriters) {
    ...
    if (varInfo.isValueBearing) valueBearingCount++;
  }

  const summary: StorageDependencySummary = {
...
function buildSharedStateMatrix(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  writeGraph?: StorageWriteGraph,
  callEdges: CallEdge[] = [],
  verbose: boolean = false
): SharedStateMatrix {
  if (verbose) console.log('[StorageDepAnalyzer] Building shared-state matrix...');

  // Identify permissionless entry points
  const entryPoints: EntryPointRow[] = [];
  const allSharedVars = new Map<string, SharedVariableColumn>();
  ...
  for (const ep of entryPoints) {
    const rowCells = new Map<string, MatrixCell>();
    ...
    for (const [varKey, varCol] of allSharedVars) {
      ...
      if (isWritten || isRead) {
        const depTypes: DependencyType[] = ['direct'];
        ...
        if (riskFactors.length > 1) highRiskCells++;
  ...
  const weakestEntryPoint = entryPoints.reduce((max, ep) => 
    ep.riskScore > max.riskScore ? ep : max, entryPoints[0]);
  
  const mostContendedVar = Array.from(allSharedVars.values()).reduce((max, v) =>
    (v.totalWriters + v.totalReaders) > (max.totalWriters + max.totalReaders) ? v : max,
    Array.from(allSharedVars.values())[0] || { variableName: 'none', contract: '', type: '', isValueBearing: false, totalReaders: 0, totalWriters: 0, entryPointReaders: 0, entryPointWriters: 0 });

  const riskSummary: MatrixRiskSummary = {
    highRiskCells,
    criticalIntersections,
...
function generateWriteExplanations(
  writeGraph: StorageWriteGraph,
  invariants: Invariant[] = [],
  verbose: boolean = false
): WriteExplanationMap {
  if (verbose) console.log('[StorageDepAnalyzer] Generating write explanations...');

  const explanations = new Map<string, WriteExplanation>();
  const unexplainedWrites: UnexplainedWrite[] = [];
  const discoveredRules: AccountingRule[] = [];
  ...
  for (const [varKey, varInfo] of writeGraph.variableWriters) {
    for (const writer of varInfo.writers) {
      ...
      const rule = findOrCreateAccountingRule(
        varInfo.variableName,
        reason,
        writer.contract,
        writer.functionName,
      ...
      const atomicity = determineAtomicityRequirement(
        varInfo.variableName,
        reason,
        varInfo.isValueBearing
      );

      // Find related invariants
      const maintainedInv = varInfo.relatedInvariants.find(inv => 
        couldMaintainInvariant(writer.functionName, inv, invariants)
      );
      
      const violatedInv = varInfo.relatedInvariants.find(inv =>
        couldViolateInvariant(writer.functionName, inv, invariants)
      );

      // Describe state transition
      const stateTransition = describeStateTransition(
        writer.functionName,
        varInfo.variableName,
        reason
      );

      const writeExpl: WriteExplanation = {
        key: explainKey,
...
export interface UnexplainedWrite {
  key: string;
  contract: string;
  function: string;
  variable: string;
...
function buildTrustBoundaryMap(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  callEdges: CallEdge[] = [],
  verbose: boolean = false
): TrustBoundaryMap {
  if (verbose) console.log('[StorageDepAnalyzer] Building trust boundary map...');

  const boundaries: TrustBoundary[] = [];
  const crossBoundaryFlows: CrossBoundaryFlow[] = [];
  const vulnerableBoundaries: VulnerableBoundary[] = [];
...
function compileHighRiskFindings(
  writeGraph: StorageWriteGraph,
  matrix: SharedStateMatrix,
  trustMap?: TrustBoundaryMap,
  verbose: boolean = false
): HighRiskFinding[] {
...
function findRegisteredFunction(
...
function findRelatedInvariants(invariants: Invariant[], varName: string): string[] {
  return invariants
    .filter(inv => inv.relatedStateVars?.includes(varName))
    .map(inv => inv.id);
}

function isValueBearingVariable(varName: string, stateVar?: StateVariable | null): boolean {
...
function inferAccountingCategory(varName: string, stateVar?: StateVariable | null): AccountingCategory | undefined {
...
function inferSyncCategory(varName: string, stateVar?: StateVariable | null): SynchronizationCategory {
  if (/balance|supply|totalSupply/i.test(varName)) return 'accounting';
  if (/owner|admin|role|hasRole/i.test(varName)) return 'authorization';
  if (/price|oracle|twap/i.test(varName)) return 'pricing';
  if (/pool|liquidity|reserve/i.test(varName)) return 'liquidity';
  if (/healthFactor|collateralRatio|ltv/i.test(varName)) return 'solvency';
  if (/pending|settled|executed|queued/i.test(varName)) return 'settlement';
  if (/ownerOf|tokenId|nft/i.test(varName)) return 'ownership';
  if (/timestamp|lastUpdate/i.test(varName)) return 'timestamp';
  if (/fee|ratio|percentage|threshold/i.test(varName)) return 'configuration';
  return 'unknown';
}
...
function isPermissionlessFunction(
  func: FunctionDef,
...
function isPermissionlessEntryPoint(
  func: FunctionDef,
  registered?: RegisteredFunction
): boolean {
...
function assessMultiWriterRisk(func: FunctionDef, varsWritten: string[]): 'critical' | 'high' | 'medium' | 'low' {
  let riskScore = 0;
  ...
  if (func.body?.ceiPattern === 'violated') riskScore += 30;
  
  // External calls add risk
  if (func.body?.hasExternalCall) riskScore += 20;
  
  // Loops can indicate unbounded iteration
  if (func.body?.hasLoop) riskScore += 15;
  
  // Delegatecall is very risky
  if (func.body?.hasDelegateCall) riskScore += 25;
...
function calculateContentionRisk(varInfo: VariableWriterInfo): number {
  let risk = 0;
  ...
  if (varInfo.isValueBearing) risk += 30;
  ...
  if (varInfo.readerFunctions.length > 0) risk += 10;
...
function assessEntryPointRisk(
  ...
  // CEI violation
  if (func.body?.ceiPattern === 'violated') risk += 25;
  
  // External calls
  if (func.body?.hasExternalCall) risk += 15;
  
  // Delegatecall
  if (func.body?.hasDelegateCall) risk += 30;
  
  return Math.min(risk, 100);
}

function inferWriteReason(
  funcName: string,
  varName: string,
  varType: string
): WriteReasonCategory {
  ...
  if (/deposit|mint|add|supply|provide/i.test(nameLower)) return 'deposit';
  if (/withdraw|burn|remove|redeem|claim/i.test(nameLower)) return 'withdrawal';
  if (/transfer|send|move|swap/i.test(nameLower)) return 'transfer';
  if (/liquidat|seize|close|force/i.test(nameLower)) return 'liquidation';
  if (/approve|permit|authorize|allow/i.test(nameLower)) return 'approval';
  if (/update.*price|set.*price|fetch.*price/i.test(nameLower)) return 'price-update';
  if (/accrue|interest|reward|yield|collect/i.test(nameLower)) return 'accrual';
  if (/init|constructor|initialize/i.test(nameLower)) return 'initialize';
  if (/own|transfer.*own/i.test(nameLower)) return 'ownership-change';
  if (/pause|unpause|enable|disable|toggle/i.test(nameLower)) return 'state-flag';
  if (/config|set.*param|update.*param/i.test(nameLower)) return 'configuration';
  if (/reconcile|sync|check|verify/i.test(nameLower)) return 'accounting';
  if (/refresh|cache|update.*cache/i.test(nameLower)) return 'cache-update';

  // Check variable name patterns
  if (/balance/i.test(varLower) && /add|inc|increase/i.test(nameLower)) return 'deposit';
  if (/balance/i.test(varLower) && /sub|dec|decrease/i.test(nameLower)) return 'withdrawal';
  if (/totalSupply/i.test(varLower)) return 'transfer';
  if (/timestamp/i.test(varLower)) return 'cache-update';
  if (/owner/i.test(varLower)) return 'ownership-change';
...
function determineAtomicityRequirement(
  varName: string,
  reason: WriteReasonCategory,
  isValueBearing: boolean
...
function couldMaintainInvariant(
  funcName: string,
  invariantId: string,
...
function couldViolateInvariant(
  funcName: string,
  invariantId: string,
  _invariants: Invariant[]
): string | undefined {
...
function describeStateTransition(
  funcName: string,
  varName: string,
  reason: WriteReasonCategory
): StateTransitionDescription {
  const transitions: Record<string, StateTransitionDescription> = {
    'deposit': {
...
function inferBoundaryType(edge: CallEdge): BoundaryType | null {
  switch (edge.type) {
    case 'delegatecall': return 'delegatecall';
    case 'staticcall': return 'library-call';
    case 'external':
      if (edge.valueFlow?.ethSent) return 'token-transfer';
      if (/transfer|transferFrom|mint|burn/i.test(edge.to.function)) return 'token-transfer';
      if (/price|oracle|getRate/i.test(edge.to.function)) return 'oracle-read';
      if (/flashLoan/i.test(edge.to.function)) return 'flash-loan';
      return 'external-call';
    default: return null;
  }
}

function inferTrustAssumption(edge: CallEdge, type: BoundaryType): string {
  switch (type) {
    case 'delegatecall':
...
function describeBoundaryBreachImpact(type: BoundaryType, edge: CallEdge): string {
  switch (type) {
    case 'delegatecall':
...
function assessBoundaryStrength(
  edge: CallEdge,
  type: BoundaryType,
  isPermissionlessSource: boolean
): 'strong' | 'medium' | 'weak' | 'none' {
  let strength = 100;

  // Delegatecall is inherently risky
  if (type === 'delegatecall') strength -= 50;
  
  // Flash loans are very risky
  if (type === 'flash-loan') strength -= 40;
  
  // Permissionless caller makes it worse
  if (isPermissionlessSource) strength -= 20;
  
  // Unknown external targets are risky
  if (edge.to.contract === 'external') strength -= 15;

  // Value transfer adds risk
  if (edge.valueFlow?.ethSent) strength -= 10;
...
function guessDataExposed(edge: CallEdge, sourceFunc?: FunctionDef): string[] {
...
function hasStateModificationAcrossBoundary(sourceFunc?: FunctionDef, _edge?: CallEdge): boolean {
...
function assessCEIOrdering(sourceFunc?: FunctionDef, _edge?: CallEdge): 'safe' | 'unsafe' | 'unknown' {
  if (!sourceFunc) return 'unknown';
  if (sourceFunc.body?.ceiPattern === 'valid') return 'safe';
  if (sourceFunc.body?.ceiPattern === 'violated') return 'unsafe';
  return 'unknown';
}

function detectBoundaryVulnerabilities(
  boundary: TrustBoundary,
  edge: CallEdge,
  sourceFunc?: FunctionDef
): VulnerableBoundary[] {
  const vulns: VulnerableBoundary[] = [];
...
function countCrossContractDeps(
  variableWriters: Map<string, VariableWriterInfo>,
  contracts: SolidityContract[]
): number {
  let count = 0;
 ...
 */
export function exportStorageDependencyResult(result: StorageDependencyAnalysisResult): any {
  return {
 ...
 */
export function generateStorageWriteGraphMermaid(graph: StorageWriteGraph): string {
 ...
 */
export function generateStorageDependencyMarkdown(result: StorageDependencyAnalysisResult): string {
