...
import { 
  ...
  VariableWriterInfo,
  WriteReasonCategory 
} from './storage-dependency-analyzer';

// ============================================================
// CORE TYPE DEFINITIONS FOR STATE COUPLING ANALYSIS
// ============================================================

export interface StateCouplingAnalysisResult {
  timestamp: string;
  functionDependencyMatrix: FunctionDependencyMatrix;
  hiddenCouplings: HiddenCouplingReport;
  invariantFunctionMap: InvariantFunctionMapping;
  variableClassification: VariableClassificationReport;
  topStateIntersections: TopIntersectionsList;
  hiddenAssumptions: HiddenAssumptionReport;
  // Quick access
  criticalFindings: CriticalFinding[];
}

// -----------------------------------------------------------
// FUNCTION DEPENDENCY MATRIX TYPES
// -----------------------------------------------------------

export interface FunctionDependencyMatrix {
  /** All functions included in analysis */
  functions: MatrixFunctionInfo[];
  /** N×N dependency matrix: key = "funcA→funcB" */
  dependencies: Map<string, DependencyRelation>;
  /** Statistics */
  statistics: MatrixStatistics;
  /** Strongly coupled clusters (functions that interact heavily) */
  couplingClusters: CouplingCluster[];
}

export interface MatrixFunctionInfo {
  id: string;                    // "Contract.functionName"
  contract: string;
  name: string;
  signature: string;
  visibility: string;
  ...
  inDegree: number;              // How many functions depend on this one's writes
  outDegree: number;             // How many functions this one depends on
  couplingScore: number;         // Overall coupling (0-100)
  riskLevel: 'critical' | 'high' | 'medium' | 'low' | 'safe';
}

export interface DependencyRelation {
  sourceFunction: string;        // "Contract.funcA"
  targetFunction: string;        // "Contract.funcB"
  dependencyType: 'write-read' | 'read-write' | 'write-write' | 'shared-state';
  sharedVariables: string[];     // Variables creating this dependency
  couplingStrength: number;      // 0-100 based on # vars + sensitivity
  isDirect: boolean;             // Direct call vs indirect via storage
...
export interface MatrixStatistics {
  ...
  averageDependenciesPerFunction: number;
  maxInDegree: number;
  maxOutDegree: number;
  stronglyCoupledPairs: number;   // Pairs with >3 shared variables
  crossContractDependencies: number;
...
export interface CouplingCluster {
  ...
  cohesionScore: number;         // How tightly coupled (0-100)
  riskLevel: 'critical' | 'high' | 'medium' | 'low';
  description: string;
}

// -----------------------------------------------------------
// HIDDEN COUPLING DETECTION TYPES
// -----------------------------------------------------------

export interface HiddenCouplingReport {
  couplings: HiddenCoupling[];
  summary: CouplingSummary;
  byCategory: Map<string, HiddenCoupling[]>;
}
...
export interface HiddenCoupling {
  ...
  source: CouplingLocation;
  target: CouplingLocation;
  mechanism: string;
  sharedState?: string[];
  description: string;
...
export type HiddenCouplingType =
  | 'inheritance-storage-overlap'     // Parent/child share storage layout
  | 'proxy-storage-conflict'          // Proxy and implementation storage clash
  | 'delegatecall-context-leak'       // Delegatecall exposes full context
  | 'cross-contract-assumed-state'    // Assumes external contract state
  | 'library-storage-sharing'         // Library accesses calling context storage
  | 'struct-layout-assumption'        // Assumes struct field ordering
  | 'immutable-pattern-violation'     // Should be immutable but isn't
  | 'storage-slot-collision'          // Manual slot assignment conflicts
  | 'callback-state-dependence'       // Callback assumes pre-callback state
  | 'multi-contract-consistency'    // Multiple contracts must stay in sync
  | 'timestamp-dependent'            // Time-based state assumption
  | 'protocol-dependent';           // Cross-protocol state assumption

export interface CouplingLocation {
  contract: string;
  function?: string;
  variable?: string;
  lineLocation?: number;
...
export interface CouplingSummary {
  totalHiddenCouplings: number;
  criticalCount: number;
  highCount: number;
  byType: Record<string, number>;
  mostAffectedContract: string;
}

// -----------------------------------------------------------
// INVARIANT FUNCTION MAPPING TYPES
// -----------------------------------------------------------

export interface InvariantFunctionMapping {
  mappings: InvariantFunctionEntry[];
  violationPaths: ViolationPath[];
  protectionGaps: ProtectionGap[];
}

export interface InvariantFunctionEntry {
  invariantId: string;
  invariantCategory: string;
  expression?: string;
  establishers: FunctionRole[];      // Functions that SET/establish this invariant
  dependers: FunctionRole[];         // Functions that RELY on this invariant
  modifiers: FunctionRole[];         // Functions that MODIFY underlying state
  potentialViolators: FunctionRole[];// Functions that COULD violate if buggy
  completeness: 'complete' | 'partial' | 'unknown';
}

export interface FunctionRole {
  functionId: string;               // "Contract.function"
  role: 'establisher' | 'depender' | 'modifier' | 'potential-violator';
  mechanism: string;                // How it interacts with the invariant
...
export interface ViolationStep {
  stepOrder: number;
  function: string;
  action: 'read' | 'write' | 'external-call' | 'check';
  variable?: string;
...
export interface VariableClassificationReport {
  classifications: ClassifiedVariable[];
  summary: ClassificationSummary;
  byCategory: Map<string, ClassifiedVariable[]>;
}
...
export interface ClassifiedVariable {
  ...
  contract: string;
  type: string;
  primaryCategory: VariableCategory;
  secondaryCategories: VariableCategory[];
  securitySensitivity: 'critical' | 'high' | 'medium' | 'low';
  integrityRequirement: 'exact' | 'bounded' | 'monotonic' | 'eventual';
...
export type VariableCategory =
  | 'accounting'           // Participates in accounting (balances, supplies)
  | 'authorization'        // Influences permissions/roles
  | 'pricing'              // Affects pricing/valuation
  | 'liquidity'            // Pool/liquidity-related
  | 'solvency'             // Health factor/collateral ratios
  | 'settlement'           // Pending/completed state tracking
  | 'configuration'        // Protocol parameters/settings
  | 'indexing'             // Used for efficient lookups
  | 'caching'              // Computed/cached values
  | 'metadata'             // Informational only
  | 'ownership'            // NFT/asset ownership
...
export interface ClassificationSummary {
  ...
  criticalSensitivityCount: number;
  byPrimaryCategory: Record<string, number>;
  mostSensitiveVariable?: string;
  mostWidelyUsedVariable?: string;
}

export interface CrossContractImpact {
  targetContract: string;
  impactType: 'read-dependent' | 'write-coordinated' | 'assumed-consistent';
  description: string;
}

// -----------------------------------------------------------
// TOP INTERSECTIONS TYPES
// -----------------------------------------------------------

export interface TopIntersectionsList {
  intersections: StateIntersection[];
  rankingMethodology: string;
  generatedAt: string;
}

export interface StateIntersection {
  rank: number;
  variables: string[];               // Variables involved
  contracts: string[];               // Contracts involved
  functions: IntersectionParticipant[];
  intersectionType: IntersectionType;
  riskScore: number;                  // 0-100
  valueAtRisk: string;                // Estimated value exposure
  exploitationComplexity: 'trivial' | 'easy' | 'moderate' | 'difficult';
  description: string;
  specificFindings: string[];
  recommendations: string[];
}

export interface IntersectionParticipant {
  functionId: string;
  role: 'writer' | 'reader' | 'both';
  accessControl: AccessControlLevel;
  isPermissionless: boolean;
  interactionType: 'direct' | 'indirect-via-call' | 'callback';
}

export type IntersectionType =
  | 'accounting-consistency'    // Must sum/balance correctly
  | 'auth-pricing-link'        // Auth affects pricing
  | 'liquidity-solvency'       // Liquidity affects solvency
  | 'settlement-atomicity'     // Must complete atomically
  | 'cross-contract-sync'      // Multiple contracts must agree
  | 'oracle-dependency'        // Depends on oracle accuracy
  | 'access-control-propagation'; // Permissions affect downstream

// -----------------------------------------------------------
// HIDDEN ASSUMPTIONS TYPES
// -----------------------------------------------------------

export interface HiddenAssumptionReport {
  assumptions: HiddenAssumption[];
  byFunction: Map<string, HiddenAssumption[]>;
  byCategory: Map<string, HiddenAssumption[]>;
...
export interface HiddenAssumption {
  ...
  heldBy: string[];                 // Functions/paths that assume this
  validatedBy: string[];             // Code that validates it (if any)
  ifWrong: string;                   // What happens if assumption is wrong
  detectability: 'easy' | 'moderate' | 'hard' | 'impossible';
...
export type AssumptionCategory =
  | 'state-freshness'           // Assumes state is current/fresh
  | 'atomicity'                 // Assumes operation is atomic
  | 'ordering'                  // Assumes execution order
  | 'external-behavior'         // Assumes external contract behavior
  | 'caller-intent'             // Assumes caller intent
  | 'math-correctness'          // Assumes math is correct
  | 'boundary-maintenance'      // Assumes boundaries are maintained
  | 'singleton-execution'       // Assumes no concurrent execution
  | 'token-compliance'          // Assumes ERC20/721 compliance
  | 'oracle-accuracy'           // Assumes oracle data is accurate
  | 'no-reentrancy'             // Assumes no reentrancy
  | 'gas-sufficiency'           // Assumes sufficient gas
  | 'timestamp-monotonicity';   // Assumes timestamps are monotonic
...
export interface AssumptionSummary {
  ...
  mostAssumptionHeavyFunction?: string;
}

// -----------------------------------------------------------
// CRITICAL FINDINGS TYPE
// -----------------------------------------------------------
...
export interface CriticalFinding {
  ...
  title: string;
  description: string;
  location: { contract: string; function?: string; variable?: string };
  evidence: string[];
  impact: string;
  remediation: string;
...
export interface StateCouplingAnalysisOptions {
  contracts: SolidityContract[];
  functionRegistry?: Map<string, RegisteredFunction[]>;
  callEdges?: CallEdge[];
  callGraph?: any;                 // Call graph from tier1-parser
  invariants?: Invariant[];
  storageWriteGraph?: StorageWriteGraph;
  verbose?: boolean;
}
 ...
 */
export function analyzeStateCouplings(
  options: StateCouplingAnalysisOptions
): StateCouplingAnalysisResult {
  const {
    contracts,
    functionRegistry,
    callEdges = [],
  ...
  if (verbose) console.log('[StateCouplingAnalyzer] Starting comprehensive state coupling analysis...');

  // Step 1: Build Function Dependency Matrix
  const functionDependencyMatrix = buildFunctionDependencyMatrix(
    contracts,
    functionRegistry,
    callEdges,
    verbose
  ...
  const hiddenCouplings = detectHiddenCouplings(
    contracts,
    functionRegistry,
    callEdges,
    verbose
  ...
  const invariantFunctionMap = buildInvariantFunctionMapping(
    contracts,
    functionRegistry,
    invariants,
    verbose
  ...
  const variableClassification = classifyVariables(
    contracts,
    functionRegistry,
    invariants,
    verbose
  ...
  const topStateIntersections = computeTopIntersections(
    functionDependencyMatrix,
    variableClassification,
    contracts,
    20,  // Top 20 as per prompt requirement
    verbose
  );

  // Step 6: Identify Hidden Assumptions
  const hiddenAssumptions = identifyHiddenAssumptions(
    contracts,
    functionRegistry,
    invariants,
    callEdges,
  ...
  const criticalFindings = compileCriticalFindings(
    functionDependencyMatrix,
    hiddenCouplings,
...
function buildFunctionDependencyMatrix(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  callEdges: CallEdge[] = [],
  verbose: boolean = false
): FunctionDependencyMatrix {
  if (verbose) console.log('[StateCouplingAnalyzer] Building function dependency matrix...');

  const functions: MatrixFunctionInfo[] = [];
  const dependencies = new Map<string, DependencyRelation>();
  ...
  for (let i = 0; i < functions.length; i++) {
    ...
    for (let j = 0; j < functions.length; j++) {
      ...
      const writtenByA_readByB = funcA.stateVariablesWritten.filter(
        v => funcB.stateVariablesRead.includes(v)
      );
      
      // Check for read-write dependency (A reads what B writes)  
      const readByA_writtenByB = funcA.stateVariablesRead.filter(
        v => funcB.stateVariablesWritten.includes(v)
      );
      
      // Check for write-write dependency (both write same var)
      const bothWrite = funcA.stateVariablesWritten.filter(
        v => funcB.stateVariablesWritten.includes(v)
      );
      
      // Determine dependency type and strength
      let depType: DependencyRelation['dependencyType'] = 'shared-state';
      let sharedVars: string[] = [];
      ...
      if (sharedVars.length > 0) {
        ...
        const strength = calculateCouplingStrength(
          sharedVars,
          funcA,
          funcB,
          isCrossContract
        ...
        const riskFactors = assessDependencyRisk(
          depType,
          sharedVars,
          funcA,
          funcB,
        ...
        const hasDirectCall = callEdges.some(e =>
          e.from.contract === funcA.contract &&
...
function calculateCouplingStrength(
  sharedVars: string[],
  funcA: MatrixFunctionInfo,
  funcB: MatrixFunctionInfo,
  isCrossContract: boolean
): number {
  let strength = 0;
  ...
  const valueBearingCount = sharedVars.filter(v => 
    /balance|supply|debt|collateral|reserve|liquidity|funds/i.test(v)
  ).length;
  strength += valueBearingCount * 10;
  
  // Permissionless functions increase risk
  if (funcA.isPermissionless) strength += 10;
  if (funcB.isPermissionless) strength += 10;
  
  // Cross-contract adds complexity
  if (isCrossContract) strength += 15;
...
function assessDependencyRisk(
  depType: DependencyRelation['dependencyType'],
  sharedVars: string[],
  funcA: MatrixFunctionInfo,
  funcB: MatrixFunctionInfo,
  isCrossContract: boolean
  ...
  switch (depType) {
    case 'write-read':
      ...
      if (funcA.isPermissionless) risks.push('permissionless-writer');
      break;
    case 'write-write':
  ...
  for (const v of sharedVars) {
    if (/balance|supply|debt/i.test(v)) risks.push('value-bearing-variable');
    if (/price|oracle|rate/i.test(v)) risks.push('price-dependency');
    if (/owner|admin|role/i.test(v)) risks.push('authorization-variable');
  }
  
  // Structural risks
  if (isCrossContract) risks.push('cross-contract-dependency');
...
function generateDependencyDescription(
  depType: DependencyRelation['dependencyType'],
  funcA: MatrixFunctionInfo,
  funcB: MatrixFunctionInfo,
  sharedVars: string[]
...
function calculateFunctionCouplingScore(
  func: MatrixFunctionInfo,
  dependencies: Map<string, DependencyRelation>
): number {
  let score = 0;
...
function assessFunctionRiskLevel(func: MatrixFunctionInfo): 'critical' | 'high' | 'medium' | 'low' | 'safe' {
  if (func.couplingScore >= 80 && func.isPermissionless) return 'critical';
  if (func.couplingScore >= 70 || (func.couplingScore >= 50 && func.isPermissionless)) return 'high';
  if (func.couplingScore >= 40) return 'medium';
  if (func.couplingScore >= 20) return 'low';
  return 'safe';
}

function findCouplingClusters(
  functions: MatrixFunctionInfo[],
  dependencies: Map<string, DependencyRelation>
): CouplingCluster[] {
  ...
  for (const func of functions) {
    ...
    if (clusterFunctions.length >= 2) {
      const clusterFuncs = clusterFunctions.map(id => 
...
function computeMatrixStatistics(
  functions: MatrixFunctionInfo[],
  dependencies: Map<string, DependencyRelation>
): MatrixStatistics {
  let maxInDegree = 0;
  ...
  for (const [, dep] of dependencies) {
    if (dep.sharedVariables.length > 3) stronglyCoupled++;
    if (dep.isCrossContract) crossContract++;
    
    // Simple cycle detection (A→B and B→A)
    const reverseKey = `${dep.targetFunction}→${dep.sourceFunction}`;
    if (dependencies.has(reverseKey)) cyclic++;
  }
...
function detectHiddenCouplings(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  callEdges: CallEdge[] = [],
  verbose: boolean = false
): HiddenCouplingReport {
  if (verbose) console.log('[StateCouplingAnalyzer] Detecting hidden couplings...');

  const couplings: HiddenCoupling[] = [];
  const byCategory = new Map<string, HiddenCoupling[]>();
  let couplingId = 0;
      ...
      for (const parentName of contract.inherited) {
        ...
        if (parent) {
          ...
          for (const childVar of contract.stateVariables) {
            const matchingParent = parent.stateVariables.find(pv =>
              pv.name.toLowerCase() === childVar.name.toLowerCase() ||
              childVar.name.toLowerCase().includes(pv.name.toLowerCase()) ||
              pv.name.toLowerCase().includes(childVar.name.toLowerCase())
            );
            
            if (matchingParent) {
              const coupling: HiddenCoupling = {
                id: `HC_${++couplingId}`,
  ...
  for (const contract of contracts) {
    for (const func of contract.functions) {
      ...
      const readsLocalThenCallsExternal = 
        (func.stateVariablesRead?.length > 0) && 
  ...
  const stateVarsByContract = new Map<string, Set<string>>();
  for (const contract of contracts) {
  ...
  for (let i = 0; i < contractNames.length; i++) {
    for (let j = i + 1; j < contractNames.length; j++) {
      ...
      const commonPatterns = Array.from(varsA).filter(va =>
        Array.from(varsB).some(vb => 
...
function categorizeCoupling(byCategory: Map<string, HiddenCoupling[]>, coupling: HiddenCoupling): void {
...
function buildInvariantFunctionMapping(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  invariants: Invariant[] = [],
  verbose: boolean = false
): InvariantFunctionMapping {
  if (verbose) console.log('[StateCouplingAnalyzer] Building invariant-function mapping...');

  const mappings: InvariantFunctionEntry[] = [];
  const violationPaths: ViolationPath[] = [];
  const protectionGaps: ProtectionGap[] = [];

  // Process each invariant
  for (const inv of invariants) {
    const entry: InvariantFunctionEntry = {
      invariantId: inv.id,
    ...
    for (const contract of contracts) {
      for (const func of contract.functions) {
        ...
        if (!writesInvVar && !readsInvVar) continue;

        const funcRole: FunctionRole = {
    ...
    for (const violator of entry.potentialViolators) {
      ...
      if (path) violationPaths.push(path);
    }
...
function generateViolationPath(
  inv: Invariant,
  violator: FunctionRole,
  contracts: SolidityContract[]
): ViolationPath | null {
...
function assessViolationFeasibility(_func: FunctionDef, violator: FunctionRole): ViolationPath['feasibility'] {
  if (violator.confidence === 'certain') return 'easy';
  if (violator.confidence === 'likely') return 'moderate';
  return 'difficult';
}
...
function classifyVariables(
  ...
  invariants: Invariant[] = [],
  verbose: boolean = false
): VariableClassificationReport {
  if (verbose) console.log('[StateCouplingAnalyzer] Classifying variables...');

  const classifications: ClassifiedVariable[] = [];
  const byCategory = new Map<string, ClassifiedVariable[]>();
  let criticalCount = 0;
  let mostSensitive: string = '';
  let mostWidelyUsed: string = '';
  let maxUsage = 0;

  for (const contract of contracts) {
    for (const stateVar of contract.stateVariables) {
      // Skip constants and immutables
      if (stateVar.visibility === 'constant' || stateVar.visibility === 'immutable') continue;
      ...
      const relatedInvariants = invariants.filter(inv =>
        inv.relatedStateVars?.includes(stateVar.name)
      ).map(inv => inv.id);
      ...
      const crossContractImpact = findCrossContractImpacts(
        stateVar.name, contract.name, contracts
      );

      const classified: ClassifiedVariable = {
...
function classifyVariablePrimary(stateVar: StateVariable): VariableCategory {
...
function classifyVariableSecondary(stateVar: StateVariable): VariableCategory[] {
  const secondary: VariableCategory[] = [];
  const name = stateVar.name.toLowerCase();

  // Many variables have secondary characteristics
  if (/mapping.*address.*uint/i.test(stateVar.type) || /balance/i.test(name)) {
    if (!secondary.includes('accounting')) secondary.push('accounting');
  }

  if (/address.*owner/i.test(name) || /address.*admin/i.test(name)) {
    if (!secondary.includes('authorization')) secondary.push('authorization');
  }

  return secondary;
}

function assessVariableSensitivity(stateVar: StateVariable, category: VariableCategory): 'critical' | 'high' | 'medium' | 'low' {
  const sensitiveCategories: VariableCategory[] = ['accounting', 'authorization', 'pricing', 'solvency', 'liquidity'];
...
function determineIntegrityRequirement(category: VariableCategory): ClassifiedVariable['integrityRequirement'] {
  switch (category) {
    ...
    case 'settlement': return 'atomic' as any; // Fix type
    case 'configuration': return 'eventual';
    case 'timestamp': return 'monotonic' as any; // Fix type
    default: return 'eventual';
  }
}

function generateClassificationRationale(stateVar: StateVariable, category: VariableCategory): string {
  const rationales: Record<VariableCategory, string> = {
...
function findCrossContractImpacts(
  varName: string,
  contractName: string,
  contracts: SolidityContract[]
): CrossContractImpact[] {
  const impacts: CrossContractImpact[] = [];
  ...
  for (const contract of contracts) {
    ...
    for (const func of contract.functions) {
      const callsTarget = func.calls?.some(call => 
        call.includes(contractName) || call.includes('.')
      );
...
function computeTopIntersections(
  depMatrix: FunctionDependencyMatrix,
  varClass: VariableClassificationReport,
  contracts: SolidityContract[],
  topN: number = 20,
  verbose: boolean = false
): TopIntersectionsList {
  if (verbose) console.log(`[StateCouplingAnalyzer] Computing top ${topN} state intersections...`);

  const candidateIntersections: StateIntersection[] = [];
  let intersectionId = 0;

  // Analyze high-value variable interactions
  const criticalVars = varClass.classifications.filter(v => 
  ...
  for (const critVar of criticalVars) {
    ...
    for (const func of depMatrix.functions) {
      if (func.contract !== critVar.contract) continue;
    ...
    if (participants.length < 2) continue; // Need at least 2 participants for intersection
  ...
  for (const cluster of depMatrix.couplingClusters) {
    if (cluster.sharedVariables.length < 2) continue;
    ...
    const clusterParticipants = cluster.functions.map(fid => {
      ...
      return func ? {
        ...
        accessControl: func.accessControl,
        isPermissionless: func.isPermissionless,
        interactionType: 'indirect-via-call' as const
      } : null;
    }).filter(Boolean) as IntersectionParticipant[];
...
function determineIntersectionType(
  varCategory: VariableCategory,
  _participants: IntersectionParticipant[]
): IntersectionType {
  switch (varCategory) {
...
function calculateIntersectionRisk(
  varInfo: ClassifiedVariable,
...
function estimateExploitationComplexity(
  participants: IntersectionParticipant[],
  riskScore: number
): StateIntersection['exploitationComplexity'] {
  const permWriterExists = participants.some(p => p.isPermissionless && 
    (p.role === 'writer' || p.role === 'both'));

  if (permWriterExists && riskScore >= 80) return 'trivial';
  if (permWriterExists && riskScore >= 60) return 'easy';
  if (riskScore >= 70) return 'moderate';
  return 'difficult';
}

function estimateValueAtRisk(varInfo: ClassifiedVariable): string {
  const valueEstimates: Record<VariableCategory, string> = {
...
function generateIntersectionFindings(
  varInfo: ClassifiedVariable,
  participants: IntersectionParticipant[]
): string[] {
...
function generateIntersectionRecommendations(
  varInfo: ClassifiedVariable,
  participants: IntersectionParticipant[],
  riskScore: number
): string[] {
...
function identifyHiddenAssumptions(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  invariants: Invariant[] = [],
  callEdges: CallEdge[] = [],
  verbose: boolean = false
): HiddenAssumptionReport {
  if (verbose) console.log('[StateCouplingAnalyzer] Identifying hidden assumptions...');

  const assumptions: HiddenAssumption[] = [];
  const byFunction = new Map<string, HiddenAssumption[]>();
  const byCategory = new Map<string, HiddenAssumption[]>();
  let assumptionId = 0;

  for (const contract of contracts) {
    for (const func of contract.functions) {
      const registeredFunc = findRegFunc(functionRegistry, contract.name, func.name);
      const funcAssumptions: HiddenAssumption[] = [];
      ...
      if (func.body?.hasExternalCall && func.body?.ceiPattern === 'violated') {
        funcAssumptions.push(createAssumption(
          ++assumptionId,
          'No reentrant call will occur during execution',
          'no-reentrancy',
          { contract: contract.name, function: func.name, line: func.lineStart },
          [`${contract.name}.${func.name}`],
          [],
          'Reentrant call can double-spend/double-execute',
          'easy',
          'trivial',
          'critical',
          'Implement CEI pattern or reentrancy guard'
        ));
      }

      // 2. Token compliance assumption (for functions calling token contracts)
      const hasTokenCall = func.calls?.some(c => 
        /transfer|transferFrom|mint|burn|approve|permit/i.test(c)
      );
      if (hasTokenCall) {
        funcAssumptions.push(createAssumption(
          ++assumptionId,
          'Token contracts follow ERC20/721 specification exactly',
          'token-compliance',
          { contract: contract.name, function: func.name },
          [`${contract.name}.${func.name}`],
          [],
          'Malicious token can block transfers, return false silently, or reenter',
          'moderate',
          'easy',
          'high',
          'Check return values, use SafeERC20, handle false returns'
        ));
      }

      // 3. Oracle accuracy assumption (for functions reading prices)
      const readsPrice = func.stateVariablesRead?.some(v => 
        /price|oracle|rate/i.test(v)
      ) || func.calls?.some(c => /price|oracle|getRate/i.test(c));
      if (readsPrice) {
        funcAssumptions.push(createAssumption(
          ++assumptionId,
          'Oracle/data source provides accurate and manipulation-resistant prices',
          'oracle-accuracy',
          { contract: contract.name, function: func.name },
          [`${contract.name}.${func.name}`],
          [],
          'Manipulated oracle causes incorrect valuation, bad debt, unfair liquidations',
          'hard',
          'moderate',
          'critical',
          'Use TWAP oracles, implement circuit breakers, add sanity checks'
        ));
      }
      ...
      if ((func.stateVariablesWritten?.length || 0) >= 2) {
        funcAssumptions.push(createAssumption(
          ++assumptionId,
          'All state updates in this function execute atomically (no interleaving)',
          'atomicity',
          { contract: contract.name, function: func.name, line: func.lineStart },
          [`${contract.name}.${func.name}`],
          [],
          'Concurrent execution could see partially-updated state',
          'hard',
          'difficult',
          func.body?.hasExternalCall ? 'high' : 'medium',
          'Use mutex pattern or ensure single-entry-point execution'
        ));
      }
      ...
      if (isPermFunc(func, registeredFunc)) {
        funcAssumptions.push(createAssumption(
          ++assumptionId,
          'Caller acts in good faith within expected use cases',
          'caller-intent',
          { contract: contract.name, function: func.name },
          [`${contract.name}.${func.name}`],
          [],
          'Adversarial caller exploits edge cases beyond intended parameters',
          'impossible',
          'trivial',
          'medium',
          'Validate all inputs rigorously, check boundary conditions'
        ));
      }
      ...
      if (func.stateVariablesRead?.some(v => /timestamp|time/i.test(v)) ||
        ...
        funcAssumptions.push(createAssumption(
          ++assumptionId,
          'Timestamps are monotonically increasing across blocks',
          'timestamp-monotonicity',
          { contract: contract.name, function: func.name },
          [`${contract.name}.${func.name}`],
          [],
          'Miner manipulation of timestamp within allowed range',
          'easy',
          'easy',
          'low',
          'Use wider time windows, avoid precise timestamp comparisons'
        ));
      }
      ...
      if (func.complexity > 10 || /mul|div|pow|sqrt/i.test(func.name)) {
        funcAssumptions.push(createAssumption(
          ++assumptionId,
          'Mathematical operations produce correct results without overflow/underflow',
          'math-correctness',
          { contract: contract.name, function: func.name, line: func.lineStart },
          [`${contract.name}.${func.name}`],
          [],
          'Overflow/underflow/precision loss causes incorrect calculations',
          'easy',
          'easy',
          'high',
          'Use SafeMath, check bounds, validate precision requirements'
        ));
      }
...
function createAssumption(
  ...
  validatedBy: string[],
  ifWrong: string,
  detectability: HiddenAssumption['detectability'],
  exploitability: HiddenAssumption['exploitability'],
  severity: HiddenAssumption['severity'],
  recommendation: string
): HiddenAssumption {
  return {
    id: `HA_${id}`,
...
function compileCriticalFindings(
  depMatrix: FunctionDependencyMatrix,
  hiddenCoups: HiddenCouplingReport,
  invMap: InvariantFunctionMapping,
  varClass: VariableClassificationReport,
  topIntersections: TopIntersectionsList,
  hiddenAssumps: HiddenAssumptionReport,
  verbose: boolean = false
): CriticalFinding[] {
...
function findRegFunc(
  registry?: Map<string, RegisteredFunction[]>,
  contract?: string,
  funcName?: string
): RegisteredFunction | undefined {
  if (!registry || !contract || !funcName) return undefined;
  const contractFuncs = registry.get(contract);
  return contractFuncs?.find(f => f.name === funcName || f.signature.includes(funcName));
}

function inferAccessControl(func: FunctionDef): AccessControlLevel {
  if (func.modifiers.some(m => /onlyOwner|onlyAdmin|onlyRole/.test(m))) return 'admin-only';
  if (func.modifiers.length > 0) return 'restricted';
  if (func.visibility === 'internal' || func.visibility === 'private') return 'internal';
  return 'public';
}

function isPermFunc(func: FunctionDef, registered?: RegisteredFunction): boolean {
  if (registered) {
    return registered.accessControl.level === 'public' && 
           (func.visibility === 'external' || func.visibility === 'public');
  }
  const hasAccessRestriction = func.modifiers.some(m =>
    /onlyOwner|onlyAdmin|onlyRole|require|whenNotPaused/.test(m)
  );
...
function findFunc(contracts: SolidityContract[], contractName: string, funcName: string): FunctionDef | undefined {
 ...
 */
export function exportStateCouplingResult(result: StateCouplingAnalysisResult): any {
  return {
 ...
 */
export function generateStateCouplingMarkdown(result: StateCouplingAnalysisResult): string {
...
// ============================================================
// FIX A: DEEP COUPLING EXPLOITATION
// Implements RedTeam-Trackator integration requirement:
// - Weaponized coupling attack generator
// - Split atomic operations vulnerability finder
// - TOCTOU exploit generator from hidden couplings
// - Attack scenario builder with pre-computed vectors
// ============================================================

// -----------------------------------------------------------
// WEAPONIZED COUPLING ATTACK TYPES
// -----------------------------------------------------------

export interface WeaponizedCouplingResult {
  timestamp: string;
  attackScenarios: WeaponizedAttack[];
  atomicityViolations: AtomicityViolation[];
  toctouAttacks: TOCTOUAttack[];
  couplingExploits: CouplingExploit[];
  summary: WeaponizationSummary;
  redteamOptimizedOutput: RedteamOptimizedAttacks;
}

export interface WeaponizedAttack {
  attackId: string;
  ...
  targetContract: string;
  targetFunctions: string[];          // Functions involved in attack
  targetVariables: string[];         // Variables being exploited
  ...
  prerequisites: AttackPrerequisite[];
  
  // Impact assessment
  impact: {
    potentialLoss: string;
    affectedUsers: 'single' | 'multiple' | 'all';
    fundDrainagePossible: boolean;
    stateCorruptionPossible: boolean;
  };
  
  // Trackator evidence sources
  trackatorEvidence: {
    couplingData?: DependencyRelation;
    hiddenCoupling?: HiddenCoupling;
    stateIntersection?: StateIntersection;
    invariantViolation?: ViolationPath;
  };
  
  // Exploitation feasibility
  feasibility: {
    complexity: 'trivial' | 'easy' | 'moderate' | 'difficult' | 'impossible';
...
export type AttackType =
  | 'atomicity-violation'           // Split atomic operations
  | 'temporal-coupling-exploit'     // TOCTOU via hidden coupling
  | 'invariant-violation-chain'     // Chain invariant violations
  | 'state-cascade-corruption'      // Corrupt state through couplings
  | 'permissionless-multi-write'    // Multiple writes without checks
  | 'reentrancy-amplification'      // Use coupling to amplify reentrancy
  | 'access-control-bypass'         // Bypass AC via coupled functions
  | 'denial-of-service-coupling'    // DoS via coupling loops;
...
export interface AttackPrerequisite {
  prerequisite: string;
  category: 'state' | 'access' | 'external' | 'timing' | 'capital';
  satisfiable: boolean;
  howToAchieve: string;
...
export interface AtomicityViolation {
  ...
  hasReentrancyGuard: { funcA: boolean; funcB: boolean };
  recommendedFix: string;
}

export interface TOCTOUAttack {
  attackId: string;
  sourceCoupling: HiddenCoupling;
  timeWindow: {
    exists: boolean;
    windowMs?: number;
  ...
  attackSequence: {
    step1: { action: string; timing: string };
    step2: { action: string; timing: string };
    step3?: { action: string; timing: string };
  };
...
export interface CouplingExploit {
  exploitId: string;
  couplingCluster: CouplingCluster;
  exploitationVector: string;
  attackSurface: {
    entryPoints: string[];
    vulnerableVariables: string[];
    missingChecks: string[];
  };
  payloadConstruction: string;
  profitExtraction: string;
  estimatedValueAtRisk: string;
}

export interface WeaponizationSummary {
  totalAttacksGenerated: number;
  criticalAttacks: number;
  highSeverityAttacks: number;
  byAttackType: Record<AttackType, number>;
  averageFeasibilityScore: number;  // 0-100
  totalValueAtRisk: string;
  topTargets: string[];            // Highest value targets
}

export interface RedteamOptimizedAttacks {
  /** Pre-computed attack surface summary for quick consumption */
  attackSurfaceSummary: AttackSurfaceItem[];
  
  /** Ranked targets by exploitability × impact */
  rankedTargets: RankedTarget[];
  
  /** Quick-reference attack patterns */
  attackPatterns: QuickAttackPattern[];
...
export interface RankedTarget {
  ...
  attackTypesAvailable: AttackType[];
  recommendedFirstAttack: AttackType;
  reason: string;
}

export interface QuickAttackPattern {
  patternName: string;
  applicableTo: string[];        // Function/variable patterns
  steps: string[];               // Quick step summary
  estimatedTimeToPoC: string;
  difficulty: 'beginner' | 'intermediate' | 'advanced' | 'expert';
}

export interface IntegrationHint {
  category: 'entry-point' | 'state-setup' | 'trigger' | 'amplify' | 'extract';
  hint: string;
  trackatorField: string;
 ...
 */
export function generateWeaponizedAttacks(
  couplingResult: StateCouplingAnalysisResult,
  options?: {
    storageWriteGraph?: any;       // From Phase 1 for context
    verbose?: boolean;
  }
): WeaponizedCouplingResult {
  const verbose = options?.verbose || false;
  
  if (verbose) console.log('[WeaponizedCoupling] Generating weaponized attack scenarios...');
  
  const attackScenarios: WeaponizedAttack[] = [];
  const atomicityViolations: AtomicityViolation[] = [];
  const toctouAttacks: TOCTOUAttack[] = [];
  const couplingExploits: CouplingExploit[] = [];
  ...
  for (const pair of strongPairs) {
    ...
    for (const viol of violations.filter(v => v.severity === 'critical' || v.severity === 'high')) {
      ...
      if (attack) attackScenarios.push(attack);
    }
  }
  ...
  const redteamOutput = buildRedteamOptimizedAttacks(
    attackScenarios,
    atomicityViolations,
    toctouAttacks,
    couplingExploits,
  ...
  // Build summary
  const summary = buildWeaponizationSummary(
    attackScenarios,
    atomicityViolations,
    toctouAttacks,
    couplingExploits
...
// -----------------------------------------------------------
// ATOMICITY VIOLATION GENERATION
// -----------------------------------------------------------

interface StronglyCoupledPair {
  funcA: MatrixFunctionInfo;
  funcB: MatrixFunctionInfo;
...
function extractStronglyCoupledPairs(matrix: FunctionDependencyMatrix): StronglyCoupledPair[] {
  const pairs: StronglyCoupledPair[] = [];
...
function generateAtomicityViolations(
  pair: StronglyCoupledPair,
  _couplingResult: StateCouplingAnalysisResult,
  storageGraph?: any
): AtomicityViolation[] {
  ...
  const valueBearingVars = pair.sharedVariables.filter(v =>
    /balance|supply|debt|collateral|reserve|fund|total|amount/i.test(v)
  );
  ...
  for (const variable of valueBearingVars) {
    const violation: AtomicityViolation = {
...
function buildAtomicityScenario(
  pair: StronglyCoupledPair,
  variable: string,
  severity: string
): string {
...
function checkReentrancyGuard(func: MatrixFunctionInfo, _storageGraph?: any): boolean {
...
function buildAtomicityFixRecommendation(
  pair: StronglyCoupledPair,
  variable: string
): string {
  const suggestions = [
...
function buildAtomicityAttack(
  violation: AtomicityViolation,
  _couplingResult: StateCouplingAnalysisResult
): WeaponizedAttack | null {
  if (violation.severity !== 'critical' && violation.severity !== 'high') return null;
...
function isTemporalCoupling(coupling: HiddenCoupling): boolean {
  const temporalTypes: string[] = [
    'timestamp-dependent',
    'callback-state-dependence',
    'multi-contract-consistency',
    'protocol-dependent'
  ];
  return temporalTypes.includes(coupling.type as string);
}

function buildTOCTOUAttack(
  coupling: HiddenCoupling,
  _couplingResult: StateCouplingAnalysisResult
): TOCTOUAttack {
...
function buildTOCTOUWeaponizedAttack(
  toctou: TOCTOUAttack,
  _couplingResult: StateCouplingAnalysisResult
): WeaponizedAttack {
  ...
  return {
    ...
    prerequisites: toctou.requiredConditions.map((req, i) => ({
      prerequisite: req,
      category: i === 0 ? 'access' : i === 1 ? 'state' : 'external' as const,
      satisfiable: true,
...
function buildClusterExploit(
  ...
  const vulnerableVars = cluster.sharedVariables.filter(v =>
    /balance|supply|collateral|debt|reserve|total|price|oracle/i.test(v)
  );
...
function buildClusterAttack(
  exploit: CouplingExploit,
  _couplingResult: StateCouplingAnalysisResult
): WeaponizedAttack {
  return {
...
function buildInvariantViolationAttack(
  vPath: ViolationPath,
  _couplingResult: StateCouplingAnalysisResult
): WeaponizedAttack {
  return {
    ...
    targetVariables: vPath.executionSteps.map(s => s.variable).filter(Boolean) as string[],
    entryPoint: {
    ...
    prerequisites: vPath.prerequisiteState.map(state => ({
      ...
      difficulty: vPath.feasibility === 'trivial' ? 'trivial' : 'easy' as const
    })),
    impact: {
      potentialLoss: vPath.impactIfViolated,
      affectedUsers: 'multiple' as const,
...
function buildRedteamOptimizedAttacks(
  attacks: WeaponizedAttack[],
  atomicityViolations: AtomicityViolation[],
  toctouAttacks: TOCTOUAttack[],
  _couplingExploits: CouplingExploit[],
  _couplingResult: StateCouplingAnalysisResult
): RedteamOptimizedAttacks {
  // Build attack surface items
  const attackSurfaceItems: AttackSurfaceItem[] = attacks.map(attack => ({
  ...
  const rankedTargets: RankedTarget[] = attackSurfaceItems
    ...
    .map(item => {
      ...
      return {
        ...
        attackTypesAvailable: [item.vulnerabilityType as AttackType],
        recommendedFirstAttack: item.vulnerabilityType as AttackType,
  ...
  const attackPatterns: QuickAttackPattern[] = [
    {
  ...
  const integrationHints: IntegrationHint[] = [
    {
...
function buildWeaponizationSummary(
  attacks: WeaponizedAttack[],
  atomicityViolations: AtomicityViolation[],
  toctouAttacks: TOCTOUAttack[],
  _couplingExploits: CouplingExploit[]
  ...
  return {
    ...
    byAttackType: byType as Record<AttackType, number>,
 ...
 */
export function getCriticalAttacks(
  weaponized: WeaponizedCouplingResult
): WeaponizedAttack[] {
 ...
 */
export function getQuickWinAttacks(
  weaponized: WeaponizedCouplingResult
): WeaponizedAttack[] {
 ...
 */
export function exportWeaponizedResults(result: WeaponizedCouplingResult): any {
  return {
    timestamp: result.timestamp,
