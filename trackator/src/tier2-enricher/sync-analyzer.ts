...
// ============================================================
// CORE TYPE DEFINITIONS FOR SYNC ANALYSIS
// ============================================================

export interface SyncAnalysisResult {
  timestamp: string;
  assumptionDependencyGraph: AssumptionDependencyGraph;
  desynchronizationAnalysis: DesynchronizationAnalysisReport;
  syncBoundaries: SyncBoundaryMap;
  topSyncRelationships: TopSyncRelationshipsList;
  // Quick access
  criticalDesyncRisks: DesyncRisk[];
}

// -----------------------------------------------------------
// PART 1: ASSUMPTION DEPENDENCY GRAPH TYPES
// -----------------------------------------------------------

export interface AssumptionDependencyGraph {
  nodes: AssumptionNode[];
  edges: AssumptionEdge[];
  producers: Map<string, ProducerInfo>;
  consumers: Map<string, ConsumerInfo>;
  verifiers: Map<string, VerifierInfo>;
  graphStatistics: GraphStatistics;
}

export interface AssumptionNode {
  id: string;                    // "Contract.variable" or invariant ID
  type: 'state-variable' | 'invariant' | 'accounting-rule' | 'protocol-property';
  name: string;
  contract: string;
  category: string;
  description: string;
  isValueBearing: boolean;
  securityClassification: 'critical' | 'high' | 'medium' | 'low';
}

export interface AssumptionEdge {
  id: string;
  sourceNode: string;            // Producer node ID
  targetNode: string;            // Consumer node ID
  edgeType: 'produces' | 'depends-on' | 'validates' | 'can-violate';
  sourceFunction: string;        // Function that creates this dependency
  targetFunction: string;        // Function that uses/depends
...
export interface ProducerInfo {
  nodeId: string;
  producerFunctions: ProducerFunction[];
  productionMechanism: ProductionMechanism;
  outputVariables: string[];
  establishedInvariants: string[];
}

export interface ProducerFunction {
  functionId: string;
  contract: string;
  name: string;
  produces: string[];             // What it produces
  mechanism: 'direct-write' | 'calculation' | 'external-fetch' | 'event-triggered';
  frequency: 'per-transaction' | 'per-block' | 'on-demand' | 'scheduled';
  accessControl: AccessControlLevel;
  isPermissionless: boolean;
}

export type ProductionMechanism =
  | 'direct-state-write'
  | 'calculated-derived'
  | 'fetched-from-oracle'
  | 'received-via-event'
...
export interface ConsumerInfo {
  nodeId: string;
  consumerFunctions: ConsumerFunction[];
  assumptionMade: string;         // What the consumer assumes about the state
  validationPerformed: string;    // What validation (if any) is performed
  isBlindTrust: boolean;          // True if no validation performed
...
export interface ConsumerFunction {
  ...
  contract: string;
  name: string;
  consumes: string[];             // What it consumes
  assumesCorrect: string;         // What it assumes
  validates: string[];            // What checks it performs
  actionIfInvalid: 'revert' | 'continue' | 'undefined';
  isCriticalPath: boolean;
}

export interface VerifierInfo {
  nodeId: string;
  verifierFunctions: VerifierFunction[];
...
export interface VerifierFunction {
  ...
  name: string;
  verifies: string[];
  mechanism: 'assertion' | 'require' | 'custom-check' | 'event-based';
  triggersOn: string[];
...
export interface GraphStatistics {
  ...
  blindTrustCount: number;        // Consumers without verification
  crossContractEdges: number;
  averageDependenciesPerNode: number;
  mostDependedNode?: string;
  leastVerifiedNode?: string;
}

// -----------------------------------------------------------
// PART 2: DESYNCHRONIZATION ANALYSIS TYPES
// -----------------------------------------------------------

export interface DesynchronizationAnalysisReport {
  synchronizationGroups: SynchronizationGroup[];
  desyncSources: DesyncSource[];
  detectedRisks: DesyncRisk[];
  accountingDrift: AccountingDriftAnalysis;
  summary: DesyncSummary;
}
...
export interface SynchronizationGroup {
  ...
  variables: SyncGroupVariable[];
  contractsInvolved: string[];
  synchronizationOwner: string;   // Who/what owns keeping these in sync
  syncPoints: SyncPoint[];
  currentSyncStatus: 'synchronized' | 'at-risk' | 'unknown';
  desyncDetection: DesyncDetectionMethod;
  restorationMethod: RestorationMethod;
  riskIfDesynchronized: string;
}

export type SyncGroupType =
  | 'global-per-user-accounting'     // totalSupply ↔ Σ userBalances
  | 'debt-collateral-matching'      // totalDebt ↔ collateral value
  | 'internal-external-balance'     // internal tracking ↔ actual token balance
  | 'price-cache-live'              // cached price ↔ live oracle price
  | 'interest-accrual'              // accrued interest ↔ actual time passed
  | 'allowance-spending'            // allowance amount ↔ actual spend
  | 'ownership-metadata'            // ownerOf ↔ token metadata
  | 'cross-contract-mirror'         // same concept mirrored across contracts
  | 'state-machine-consistency'     // multiple state flags must be consistent
  | 'unknown-pattern';

export interface SyncGroupVariable {
  variableName: string;
  contract: string;
  role: 'primary' | 'derived' | 'mirrored' | 'cached';
  updateFrequency: 'high' | 'medium' | 'low' | 'on-demand';
  updaterFunctions: string[];
  readerFunctions: string[];
  lastWriter?: string;               // Most recent writer in execution context
}
...
export interface SyncPoint {
  ...
  atomicity: 'atomic' | 'eventually-consistent' | 'best-effort';
  orderingGuarantee: 'strict' | 'loose' | 'none';
}

export type SyncMechanism =
  | 'single-transaction'
  | 'multi-call-same-tx'
  | 'cross-transaction'
  | 'event-driven'
...
export type DesyncDetectionMethod =
  | 'invariant-check'
  | 'periodic-audit'
  | 'event-monitoring'
  | 'none'
...
export interface DesyncSource {
  sourceId: string;
  sourceType: DesyncSourceType;
  location: { contract: string; function?: string; variable?: string };
  affectedGroups: string[];
  description: string;
  triggerCondition: string;
  detectionDifficulty: 'easy' | 'moderate' | 'hard' | 'impossible';
  severity: 'critical' | 'high' | 'medium' | 'low';
  mitigation: string;
}

export type DesyncSourceType =
  | 'non-atomic-update'           // Multi-slot update not atomic
  | 'deferred-update'             // Update scheduled for later
  | 'cross-contract-update'       // Updates span contracts non-atomically
  | 'multi-transaction'           // Related updates across transactions
  | 'lazy-evaluation'             // Value computed on demand, may be stale
  | 'cached-staleness'            // Cached value not refreshed
  | 'partial-update'              // Only some related vars updated
  | 'asymmetric-update'           // A updated but B not (or vice versa)
  | 'callback-interference'       // Callback modified assumed state
  | 'reentrancy-gap'              // Reentrancy caused intermediate state exposure
  | 'oracle-divergence'           // Oracle values diverged from on-chain state
  | 'rounding-error'              // Precision loss accumulated
  | 'overflow-underflow'          // Math error caused desync
  | 'external-failure'            // External call failed but partial state changed;

export interface DesyncRisk {
  riskId: string;
  riskType: DesyncRiskType;
...
export type DesyncRiskType =
  | 'temporary-desync'            // Self-correcting after completion
  | 'persistent-desync'           // Requires explicit correction
  | 'accumulating-drift'          // Gets worse over time
  | 'atomicity-violation'         // Partial update visible
  | 'stale-state-usage'           // Acting on old data
  | 'inconsistent-view'           // Different functions see different states;

export interface AccountingDriftAnalysis {
  trackedPairs: AccountingPair[];
  driftDetected: boolean;
  potentialDriftLocations: DriftLocation[];
}
...
export interface DesyncSummary {
  ...
  atRiskGroups: number;
  totalDesyncSources: number;
  criticalDesyncSources: number;
  totalRisks: number;
  criticalRisks: number;
  mostVulnerableGroup?: string;
  accountingDriftRisk: 'none' | 'low' | 'medium' | 'high';
}

// -----------------------------------------------------------
// PART 3: SYNC BOUNDARY TYPES
// -----------------------------------------------------------

export interface SyncBoundaryMap {
  boundaries: SyncBoundary[];
  boundaryInteractions: BoundaryInteraction[];
  vulnerableBoundaries: VulnerableSyncBoundary[];
}

export interface SyncBoundary {
  boundaryId: string;
  boundaryType: SyncBoundaryType;
  sourceContext: string;
  targetContext: string;
  synchronizationMechanism: SyncBoundaryMechanism;
  dataCrossing: SyncDataDescription[];
...
export type SyncBoundaryType =
  | 'external-call-boundary'
  | 'delegatecall-boundary'
  | 'contract-boundary'
  | 'oracle-boundary'
...
export type SyncBoundaryMechanism =
...
export interface SyncDataDescription {
  dataType: 'account-balance' | 'price' | 'permission' | 'configuration' | 'state-flag' | 'timestamp';
  variableName?: string;
  direction: 'outbound' | 'inbound' | 'bidirectional';
  freshnessRequirement: 'real-time' | 'recent' | 'eventual' | 'static';
  consistencyRequired: 'exact' | 'approximate' | 'ordered';
}

export interface TimingRequirement {
  maxDelay: string;                // e.g., "0", "1 block", "same transaction"
  ordering: 'strict' | 'loose' | 'none';
  finalityRequirement: 'instant' | 'delayed' | 'never';
}

export interface FailureMode {
  type: 'silent-desync' | 'detectable-failure' | 'partial-completion' | 'deadlock';
  detectionMethod: string;
...
export interface RecoveryMechanism {
  type: 'auto-revert' | 'retry' | 'manual-fix' | 'upgrade-required' | 'none';
  description: string;
...
export interface BoundaryInteraction {
  ...
  fromBoundary: string;
  toBoundary: string;
  interactionType: 'sequential' | 'parallel' | 'conflicting';
  dataDependency: string;
  raceConditionRisk: boolean;
  description: string;
}

export interface VulnerableSyncBoundary {
  boundaryId: string;
...
// -----------------------------------------------------------
// PART 4: TOP SYNC RELATIONSHIPS TYPES
// -----------------------------------------------------------

export interface TopSyncRelationshipsList {
...
export interface SyncRelationship {
  ...
  relationshipType: SyncGroupType;
  variables: SyncVariableDetail[];
  producerFunctions: ProducerFunction[];
  consumerFunctions: ConsumerFunction[];
  synchronizationOwner: string;
  syncPoints: SyncPoint[];
  crossContractDependencies: CrossContractSyncDep[];
  exactSourceLocations: SourceLocation[];
  whyCritical: string;
...
export interface SyncVariableDetail {
  name: string;
  contract: string;
  type: string;
  role: 'primary' | 'derived' | 'mirrored' | 'cached';
...
export interface CrossContractSyncDep {
  sourceContract: string;
  targetContract: string;
  variable: string;
  dependencyType: 'must-match' | 'must-be-less-than' | 'must-be-greater-than' | 'ordered-update';
...
export interface SyncAnalysisOptions {
...
export function analyzeSynchronization(
  ...
  if (verbose) console.log('[SyncAnalyzer] Starting comprehensive synchronization analysis...');

  // Part 1: Build Assumption Dependency Graph
  const assumptionDependencyGraph = buildAssumptionDependencyGraph(
    contracts,
    functionRegistry,
    invariants,
    verbose
  ...
  const desynchronizationAnalysis = analyzeDesynchronization(
    contracts,
    functionRegistry,
    callEdges,
    invariants,
  ...
  const syncBoundaries = mapSyncBoundaries(
    contracts,
    functionRegistry,
    callEdges,
    verbose
  ...
  const topSyncRelationships = rankTopSyncRelationships(
...
function buildAssumptionDependencyGraph(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  invariants: Invariant[] = [],
  verbose: boolean = false
): AssumptionDependencyGraph {
  if (verbose) console.log('[SyncAnalyzer] Building assumption dependency graph...');

  const nodes: AssumptionNode[] = [];
  const edges: AssumptionEdge[] = [];
  const producers = new Map<string, ProducerInfo>();
  const consumers = new Map<string, ConsumerInfo>();
  const verifiers = new Map<string, VerifierInfo>();
  ...
  for (const contract of contracts) {
    for (const func of contract.functions) {
      ...
      for (const varName of writtenVars) {
        ...
        if (prodInfo) {
          const prodFunc: ProducerFunction = {
        ...
        for (const readVar of readVars) {
          ...
          if (!readNode || readNode.id === varNode.id) continue;
        ...
        for (const otherContract of contracts) {
          for (const otherFunc of otherContract.functions) {
            if (otherContract.name === contract.name && otherFunc.name === func.name) continue;
            ...
            if (otherReadVars.includes(varName)) {
              ...
              if (consInfo) {
                const consFunc: ConsumerFunction = {
      ...
      if (isPureValidator) {
        for (const varName of readVars) {
          ...
          if (verInfo) {
            ...
            const verFunc: VerifierFunction = {
              functionId: `${contract.name}.${func.name}`,
              contract: contract.name,
              name: func.name,
              verifies: [varName],
  ...
  for (const [, cons] of consumers) {
    if (cons.isBlindTrust) blindTrustCount++;
  }

  for (const edge of edges) {
    if (edge.crossContract) crossContractEdges++;
    
    const count = depCounts.get(edge.targetNode) || 0;
    depCounts.set(edge.targetNode, count + 1);
  }

  let mostDepended: string | undefined;
  let maxDeps = 0;
  ...
  let leastVerified: string | undefined;
...
function categorizeVariableForGraph(stateVar: StateVariable): string {
  const name = stateVar.name.toLowerCase();
  if (/balance|supply|debt/i.test(name)) return 'accounting';
  if (/owner|admin|role/i.test(name)) return 'authorization';
  if (/price|oracle|rate/i.test(name)) return 'pricing';
  if (/pool|reserve|liquidity/i.test(name)) return 'liquidity';
  if (/healthFactor|collateral/i.test(name)) return 'solvency';
  return 'general-state';
}

function inferProductionMechanism(func: FunctionDef, _varName: string): ProducerFunction['mechanism'] {
  if (/fetch|get|pull|read|load/i.test(func.name)) return 'external-fetch';
  if (/calc|compute|derive|update/i.test(func.name)) return 'calculation';
  if (/init|constructor/i.test(func.name)) return 'direct-write';
  return 'direct-write';
}

function inferUpdateFrequency(func: FunctionDef): ProducerFunction['frequency'] {
  if (/accrue|compound|tick|update/i.test(func.name)) return 'per-block';
  if (/deposit|withdraw|transfer|mint|burn/i.test(func.name)) return 'per-transaction';
  if (/refresh|cache|sync/i.test(func.name)) return 'on-demand';
...
function analyzeDesynchronization(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  callEdges: CallEdge[] = [],
  invariants: Invariant[] = [],
  verbose: boolean = false
): DesynchronizationAnalysisReport {
  if (verbose) console.log('[SyncAnalyzer] Analyzing desynchronization risks...');
...
function detectSynchronizationGroups(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  invariants: Invariant[] = []
  ...
  if (supplyVars.length > 0 && balanceVars.length > 0) {
    groups.push(createSyncGroup(
      ++groupId,
      'Global Accounting Consistency',
      'global-per-user-accounting',
      [...supplyVars, ...balanceVars],
      contracts,
      'Protocol must ensure sum of user balances equals total supply',
  ...
  if (debtVars.length > 0 && collateralVars.length > 0) {
    groups.push(createSyncGroup(
      ...
      'debt-collateral-matching',
      [...debtVars, ...collateralVars],
      contracts,
      'Total debt must be covered by collateral at all times',
      ['borrow', 'repay', 'liquidate', 'depositCollateral', 'withdrawCollateral'],
      functionRegistry
    ));
  }
  ...
  if (cachedPriceVars.length > 0 && oracleAccessFuncs.length > 0) {
    groups.push(createSyncGroup(
      ...
      cachedPriceVars,
      contracts,
      'Cached prices must stay synchronized with live oracle values',
      oracleAccessFuncs.map(f => f.name),
      functionRegistry
    ));
  }
  ...
  if (allowanceVars.length > 0 && transferFuncs.length > 0) {
    groups.push(createSyncGroup(
      ++groupId,
      'Allowance-Spending Consistency',
      'allowance-spending',
      allowanceVars,
      contracts,
      'Spending must not exceed approved allowance',
      transferFuncs.map(f => f.name),
      functionRegistry
    ));
  }
  ...
  if (interestVars.length > 0) {
    groups.push(createSyncGroup(
      ...
      interestVars,
      contracts,
      'Accrued interest/rewards must match elapsed time and rates',
      timeBasedFuncs.length > 0 ? timeBasedFuncs.map(f => f.name) : ['any-time-based-operation'],
      functionRegistry
    ));
  }
...
function createSyncGroup(
  ...
  name: string,
  type: SyncGroupType,
  variables: Array<{ name: string; contract: string; type: string }>,
  contracts: SolidityContract[],
  riskIfDesynchronized: string,
  relevantFunctionNames: string[],
  functionRegistry?: Map<string, RegisteredFunction[]>
): SynchronizationGroup {
  // Find updater and reader functions for each variable
  const syncVars: SyncGroupVariable[] = [];
...
function assessGroupStatus(group: SynchronizationGroup, contracts: SolidityContract[], functionRegistry?: Map<string, RegisteredFunction[]>): void {
  let riskFactors = 0;
...
function identifyDesyncSources(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  callEdges: CallEdge[] = [],
  syncGroups: SynchronizationGroup[] = []
): DesyncSource[] {
  const sources: DesyncSource[] = [];
  let sourceId = 0;
...
function detectDesyncRisks(
  sources: DesyncSource[],
...
function analyzeAccountingDrift(
  contracts: SolidityContract[],
  syncGroups: SynchronizationGroup[],
  functionRegistry?: Map<string, RegisteredFunction[]>
): AccountingDriftAnalysis {
  const pairs: AccountingPair[] = [];
  const driftLocations: DriftLocation[] = [];

  // Find accounting-related sync groups
  const accountingGroups = syncGroups.filter(g =>
    g.groupType === 'global-per-user-accounting' ||
...
function mapSyncBoundaries(
  contracts: SolidityContract[],
  functionRegistry?: Map<string, RegisteredFunction[]>,
  callEdges: CallEdge[] = [],
  verbose: boolean = false
): SyncBoundaryMap {
  if (verbose) console.log('[SyncAnalyzer] Mapping synchronization boundaries...');

  const boundaries: SyncBoundary[] = [];
  const interactions: BoundaryInteraction[] = [];
  const vulnerableBoundaries: VulnerableSyncBoundary[] = [];
  let boundaryId = 0;
  ...
  for (const contract of contracts) {
    for (const func of contract.functions) {
      ...
      if (isPermFunc(func, registeredFunc) && 
        ...
        const existingBoundary = boundaries.find(b => 
          b.sourceContext === contract.name && 
...
function rankTopSyncRelationships(
  depGraph: AssumptionDependencyGraph,
  desyncAnalysis: DesynchronizationAnalysisReport,
  syncBounds: SyncBoundaryMap,
  topN: number = 20,
  verbose: boolean = false
): TopSyncRelationshipsList {
  if (verbose) console.log(`[SyncAnalyzer] Ranking top ${topN} sync relationships...`);

  const candidates: SyncRelationship[] = [];
  let relId = 0;

  // Generate relationships from sync groups
  for (const group of desyncAnalysis.synchronizationGroups) {
    const rel: SyncRelationship = {
      rank: 0,
...
function findVariablesByPattern(contracts: SolidityContract[], pattern: RegExp): Array<{ name: string; contract: string; type: string }> {
  const results: Array<{ name: string; contract: string; type: string }> = [];
...
function findCrossContractMirrorPatterns(contracts: SolidityContract[]): Array<{ variableName: string; variables: Array<{ name: string; contract: string; type: string; isPrimary: boolean }> }> {
  const patterns: Array<{ variableName: string; variables: Array<{ name: string; contract: string; type: string; isPrimary: boolean }> }> = [];
  const varMap = new Map<string, Array<{ name: string; contract: string; type: string }>>();
...
function determineSyncOwner(variables: SyncGroupVariable[], contracts: SolidityContract[], functionRegistry?: Map<string, RegisteredFunction[]>): string {
  // The contract with the most updaters or the "primary" variable's contract is likely the owner
  const contractUpdaterCounts = new Map<string, number>();
...
function findAffectedSyncGroups(groups: SynchronizationGroup[], variables: string[]): SynchronizationGroup[] {
  return groups.filter(group =>
    group.variables.some(v => variables.includes(v.variableName))
  );
}

function hasCallbackPattern(func: FunctionDef): boolean {
  return /callback|onReceive|handle|_hook|_call/i.test(func.name) ||
         func.calls?.some(c => /callback|receive|hook/i.test(c));
}

function getExpectedRelationship(primaryVar: string, derivedVar: string): string {
...
function classifySyncBoundaryType(edge: CallEdge): SyncBoundary['boundaryType'] {
  switch (edge.type) {
    case 'delegatecall': return 'delegatecall-boundary';
    case 'staticcall': return 'contract-boundary';
    case 'external':
      if (/price|oracle|getRate/i.test(edge.to.function)) return 'oracle-boundary';
      if (/transfer|transferFrom/i.test(edge.to.function)) return 'token-contract-boundary';
      if (/flashLoan/i.test(edge.to.function)) return 'flash-loan-boundary';
      if (/execute|queue|castVote/i.test(edge.to.function)) return 'governance-boundary';
      if (/timelock|schedule|delay/i.test(edge.to.function)) return 'timelock-boundary';
      return 'external-call-boundary';
    default: return 'contract-boundary';
  }
}

function inferSyncMechanism(edge: CallEdge): SyncBoundary['synchronizationMechanism'] {
  if (edge.type === 'delegatecall') return 'immediate-consistency';
  if (edge.valueFlow?.ethSent) return 'immediate-consistency';
  return 'eventual-consistency';
}

function inferDataCrossing(edge: CallEdge): SyncBoundary['dataCrossing'] {
  const data: SyncDataDescription[] = [];
...
function inferTimingRequirement(_edge: CallEdge, _type: SyncBoundary['boundaryType']): TimingRequirement {
  return {
...
function inferFailureMode(_edge: CallEdge): FailureMode {
  return {
    type: 'detectable-failure',
...
function inferRecoveryMechanism(_edge: CallEdge): RecoveryMechanism {
  return {
    type: 'auto-revert',
...
function assessSyncBoundaryStrength(edge: CallEdge): SyncBoundary['strength'] {
  let strength = 100;
  
  if (edge.type === 'delegatecall') strength -= 40;
  if (edge.to.contract === 'external') strength -= 20;
...
function detectSyncBoundaryVulnerabilities(boundary: SyncBoundary, edge: CallEdge): VulnerableSyncBoundary[] {
  const vulns: VulnerableSyncBoundary[] = [];
...
function assessBoundaryInteraction(a: SyncBoundary, b: SyncBoundary): BoundaryInteraction | null {
 ...
 */
export function exportSyncAnalysisResult(result: SyncAnalysisResult): any {
  return {
 ...
 */
export function generateSyncAnalysisMarkdown(result: SyncAnalysisResult): string {
