...
export interface BreakdownIngestOptions {
 ...
 */
export function ingestBreakdown(options: BreakdownIngestOptions): BreakdownOutput {
  const {
    breakdownFile,
    contracts = [],
    verbose = false
  ...
  if (breakdownFile && fs.existsSync(breakdownFile)) {
    if (verbose) console.log(`Loading Breakdown data from ${breakdownFile}`);
  ...
  if (!rawData && contracts.length > 0) {
    if (verbose) console.log('Generating Breakdown analysis from contracts');
 ...
 */
function loadBreakdownFile(filePath: string): any {
 ...
 */
function generateBreakdownFromContracts(contracts: SolidityContract[]): any {
  // Analyze components
...
function analyzeComponents(contracts: SolidityContract[]): ComponentAnalysis[] {
...
function determineComponentType(
  contract: SolidityContract,
  _allContracts: SolidityContract[]
...
function determineResponsibility(contract: SolidityContract): string {
...
function extractInterfaces(contract: SolidityContract): ComponentAnalysis['interfaces'] {
  const interfaces: ComponentAnalysis['interfaces'] = [];
...
function assessComponentRisk(contract: SolidityContract): ComponentAnalysis['riskLevel'] {
  let riskScore = 0;
  ...
  for (const func of contract.functions) {
    ...
    if (func.body.hasExternalCall && func.stateVariablesWritten.length > 0) riskScore += 15;
    if (func.body.hasLoop) riskScore += 10;
...
function extractMoneyFlows(contracts: SolidityContract[]): MoneyFlow[] {
...
function extractFunctionMoneyFlows(
  contract: SolidityContract,
  func: FunctionDef,
  startId: number
): MoneyFlow[] {
...
function analyzeNumericBehaviors(contracts: SolidityContract[]): NumericBehavior[] {
  const behaviors: NumericBehavior[] = [];
...
function inferNumericBehavior(
  contractName: string,
  sv: { name: string; type: string }
): NumericBehavior {
...
function mapStateAuthority(contracts: SolidityContract[]): StateAuthorityMap[] {
  const authorities: StateAuthorityMap[] = [];
...
function findVariableWriters(variableName: string, contract: SolidityContract): string[] {
...
function findInternalCallers(funcName: string, contract: SolidityContract): string[] {
...
function inferChangePatterns(variableName: string, contract: SolidityContract): string {
...
function findConstraints(variableName: string, contract: SolidityContract): string[] {
...
function checkValidationPresence(_variableName: string, _contract: SolidityContract): boolean {
...
function generateExpectedBehaviors(contracts: SolidityContract[]): ExpectedBehavior[] {
...
function generateFunctionExpectedBehavior(
  func: FunctionDef,
  _contract: SolidityContract
): ExpectedBehavior {
  const name = func.name.toLowerCase();
  ...
  // State changes
  const stateChanges: StateChangeExpectation[] = [];
...
function parseComponents(raw: any[], contracts: SolidityContract[]): ComponentAnalysis[] {
  ...
  return raw.map((c: any) => ({
    ...
    interfaces: (c.interfaces || []).map((i: any) => ({
      name: i.name || '',
      inputs: i.inputs || [],
      outputs: i.outputs || [],
...
function parseMoneyFlows(raw: any[]): MoneyFlow[] {
  return raw.map((mf: any, index: number) => ({
    id: mf.id || `MF_${index + 1}`,
    name: mf.name || 'Unnamed Flow',
    trigger: mf.trigger || '',
    steps: (mf.steps || []).map((s: any) => ({
      order: s.order || 0,
      action: s.action || '',
      from: s.from || '',
      to: s.to || '',
...
function parseNumericBehaviors(raw: any[]): NumericBehavior[] {
  return raw.map((nb: any) => ({
    ...
    examples: (nb.examples || []).map((ex: any) => ({
...
function parseStateAuthority(raw: any[]): StateAuthorityMap[] {
  return raw.map((sa: any) => ({
...
function parseExpectedBehaviors(raw: any[]): ExpectedBehavior[] {
  return raw.map((eb: any) => ({
    functionSig: eb.functionSig || eb.signature || '',
    preConditions: eb.preConditions || [],
    postConditions: eb.postConditions || [],
    stateChanges: (eb.stateChanges || []).map((sc: any) => ({
...
function createEmptyBreakdownOutput(): BreakdownOutput {
  return {
    components: [],
    moneyFlows: [],
    numericBehaviors: [],
